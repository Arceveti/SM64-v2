// 0x0700FE14 - 0x0701025E
const Collision wf_seg7_collision_tower[] = {
    COL_INIT(),
    COL_VERTEX_INIT(0x41),
    COL_VERTEX(-438, 1792,  184),
    COL_VERTEX(-438, 1741,  184),
    COL_VERTEX(-438, 1741, -184),
    COL_VERTEX(-184, 1792,  438),
    COL_VERTEX(-184, 1741,  438),
    COL_VERTEX(-128, 1741, -128),
    COL_VERTEX(-128, 1741,  128),
    COL_VERTEX(-128, 1997,  128),
    COL_VERTEX(-128, 1997, -128),
    COL_VERTEX( 128, 1741,  128),
    COL_VERTEX( 184, 1741,  438),
    COL_VERTEX( 128, 1997, -128),
    COL_VERTEX( 128, 1741, -128),
    COL_VERTEX(-184, 1741, -438),
    COL_VERTEX(-438, 1792, -184),
    COL_VERTEX(-184, 1792, -438),
    COL_VERTEX( 184, 1741, -438),
    COL_VERTEX( 128, 1997,  128),
    COL_VERTEX( 438, 1741, -184),
    COL_VERTEX( 184, 1792, -438),
    COL_VERTEX( 438, 1792, -184),
    COL_VERTEX( 438, 1741,  184),
    COL_VERTEX( 438, 1792,  184),
    COL_VERTEX( 184, 1792,  438),
    COL_VERTEX(-264, 1997, -158),
    COL_VERTEX(   0, 2509,    0),
    COL_VERTEX(-158, 1997, -264),
    COL_VERTEX( 154, 1997, -264),
    COL_VERTEX(-158, 1997,  256),
    COL_VERTEX(-264, 1997,  152),
    COL_VERTEX( 171, 1997,  256),
    COL_VERTEX( 260, 1997,  152),
    COL_VERTEX( 260, 1997, -158),
    COL_VERTEX( 215, 1792,  512),
    COL_VERTEX(-215, 1792,  512),
    COL_VERTEX( 512, 1792,  215),
    COL_VERTEX(-512,    0, -215),
    COL_VERTEX(-512,    0,  215),
    COL_VERTEX(-215,    0, -512),
    COL_VERTEX(-512,  205,  108),
    COL_VERTEX(-215,    0,  512),
    COL_VERTEX( 215,    0,  512),
    COL_VERTEX( 512,    0,  215),
    COL_VERTEX( 512,    0, -215),
    COL_VERTEX( 215,    0, -512),
    COL_VERTEX( 512, 1792, -215),
    COL_VERTEX( 215, 1792, -512),
    COL_VERTEX(-215, 1792, -512),
    COL_VERTEX(-512, 1536, -215),
    COL_VERTEX(-512,  205, -108),
    COL_VERTEX(-512, 1792, -215),
    COL_VERTEX(-512, 1792,  215),
    COL_VERTEX(-512,    0, -108),
    COL_VERTEX(-307,    0, -108),
    COL_VERTEX( 215, 1536,  512),
    COL_VERTEX( 512, 1536,  215),
    COL_VERTEX( 512, 1536, -215),
    COL_VERTEX(-215, 1536,  512),
    COL_VERTEX(-215, 1536, -512),
    COL_VERTEX( 215, 1536, -512),
    COL_VERTEX(-512, 1536,  215),
    COL_VERTEX(-512,    0,  108),
    COL_VERTEX(-307,    0,  108),
    COL_VERTEX(-307,  205,  108),
    COL_VERTEX(-307,  205, -108),
    COL_TRI_INIT(SURFACE_DEFAULT, 116),
    COL_TRI(13,  5, 12),
    COL_TRI( 0,  1,  2),
    COL_TRI( 3,  4,  1),
    COL_TRI( 3,  1,  0),
    COL_TRI( 1,  5,  2),
    COL_TRI( 1,  6,  5),
    COL_TRI( 4,  6,  1),
    COL_TRI( 6,  7,  8),
    COL_TRI( 9,  7,  6),
    COL_TRI( 6,  8,  5),
    COL_TRI(10,  9,  6),
    COL_TRI(10,  6,  4),
    COL_TRI( 5, 11, 12),
    COL_TRI( 5,  8, 11),
    COL_TRI( 2,  5, 13),
    COL_TRI( 0,  2, 14),
    COL_TRI(14,  2, 13),
    COL_TRI(14, 13, 15),
    COL_TRI(15, 13, 16),
    COL_TRI(13, 12, 16),
    COL_TRI(12, 17,  9),
    COL_TRI(12, 11, 17),
    COL_TRI(18, 12,  9),
    COL_TRI(16, 12, 18),
    COL_TRI(15, 16, 19),
    COL_TRI(19, 16, 18),
    COL_TRI(19, 18, 20),
    COL_TRI(20, 18, 21),
    COL_TRI(18,  9, 21),
    COL_TRI( 9, 17,  7),
    COL_TRI(21,  9, 10),
    COL_TRI(22, 21, 10),
    COL_TRI(20, 21, 22),
    COL_TRI(23, 10,  4),
    COL_TRI(22, 10, 23),
    COL_TRI(23,  4,  3),
    COL_TRI(24, 29, 25),
    COL_TRI(25, 26, 24),
    COL_TRI( 8, 24, 26),
    COL_TRI(24,  8,  7),
    COL_TRI(24,  7, 29),
    COL_TRI( 8, 26, 27),
    COL_TRI( 8, 27, 11),
    COL_TRI(28,  7, 17),
    COL_TRI(29,  7, 28),
    COL_TRI(25, 29, 28),
    COL_TRI(30, 25, 28),
    COL_TRI(28, 17, 30),
    COL_TRI(17, 31, 30),
    COL_TRI(11, 31, 17),
    COL_TRI(30, 31, 25),
    COL_TRI(32, 25, 31),
    COL_TRI(11, 32, 31),
    COL_TRI(27, 32, 11),
    COL_TRI(32, 27, 25),
    COL_TRI(25, 27, 26),
    COL_TRI(33, 35, 22),
    COL_TRI(36, 58, 38),
    COL_TRI(34, 54, 33),
    COL_TRI(33, 54, 55),
    COL_TRI(33, 55, 35),
    COL_TRI(34, 57, 54),
    COL_TRI(35, 55, 56),
    COL_TRI(22, 35, 45),
    COL_TRI(35, 56, 45),
    COL_TRI(36, 48, 58),
    COL_TRI(37, 60, 48),
    COL_TRI(38, 58, 59),
    COL_TRI(38, 59, 44),
    COL_TRI(39, 37, 48),
    COL_TRI(39, 61, 37),
    COL_TRI(40, 60, 37),
    COL_TRI(40, 57, 60),
    COL_TRI(41, 57, 40),
    COL_TRI(45, 59, 46),
    COL_TRI(41, 54, 57),
    COL_TRI(42, 54, 41),
    COL_TRI(42, 55, 54),
    COL_TRI(43, 55, 42),
    COL_TRI(43, 56, 55),
    COL_TRI(44, 56, 43),
    COL_TRI(44, 59, 56),
    COL_TRI(45, 56, 59),
    COL_TRI(22, 45, 20),
    COL_TRI(50, 48, 60),
    COL_TRI(45, 46, 19),
    COL_TRI(46, 47, 15),
    COL_TRI(46, 15, 19),
    COL_TRI(47, 14, 15),
    COL_TRI(47, 50, 14),
    COL_TRI(47, 58, 48),
    COL_TRI(47, 48, 50),
    COL_TRI(48, 52, 49),
    COL_TRI(49, 39, 48),
    COL_TRI(48, 36, 52),
    COL_TRI(34, 23,  3),
    COL_TRI(34, 33, 23),
    COL_TRI(45, 19, 20),
    COL_TRI(50, 51,  0),
    COL_TRI(50,  0, 14),
    COL_TRI(50, 60, 51),
    COL_TRI(51, 60, 57),
    COL_TRI(51, 34,  3),
    COL_TRI(51,  3,  0),
    COL_TRI(51, 57, 34),
    COL_TRI(53, 63, 64),
    COL_TRI(33, 22, 23),
    COL_TRI(46, 59, 58),
    COL_TRI(46, 58, 47),
    COL_TRI(52, 64, 49),
    COL_TRI(52, 53, 64),
    COL_TRI(49, 64, 63),
    COL_TRI(49, 63, 39),
    COL_TRI(39, 62, 61),
    COL_TRI(39, 63, 62),
    COL_TRI(53, 62, 63),
    COL_TRI_STOP(),
    COL_END(),
};
