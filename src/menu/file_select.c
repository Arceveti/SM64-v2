...
void bhv_menu_button_loop(void) {
    switch (o->oMenuButtonState) {
        ...
        case MENU_BUTTON_STATE_GROWING: // Switching from button to menu state
            if (sCurrentMenuLevel == MENU_LAYER_MAIN   ) bhv_menu_button_growing_from_main_menu(o);
            if (sCurrentMenuLevel == MENU_LAYER_SUBMENU) bhv_menu_button_growing_from_submenu(  o); // Only used for score files
            sTextBaseAlpha       = 0;
        ...
        case MENU_BUTTON_STATE_SHRINKING: // Switching from menu to button state
            if (sCurrentMenuLevel == MENU_LAYER_MAIN   ) bhv_menu_button_shrinking_to_main_menu(o);
            if (sCurrentMenuLevel == MENU_LAYER_SUBMENU) bhv_menu_button_shrinking_to_submenu(  o); // Only used for score files
            sTextBaseAlpha       = 0;
            sCursorClickingTimer = 4;
            break;
...
void return_to_main_menu(s16 prevMenuButtonID, struct Object *sourceButton) {
    ...
    if (sMainMenuButtons[prevMenuButtonID]->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        ...
        if (prevMenuButtonID == MENU_BUTTON_SCORE     ) for ((buttonID = MENU_BUTTON_SCORE_MIN ); (buttonID < MENU_BUTTON_SCORE_MAX ); buttonID++) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
        if (prevMenuButtonID == MENU_BUTTON_COPY      ) for ((buttonID = MENU_BUTTON_COPY_MIN  ); (buttonID < MENU_BUTTON_COPY_MAX  ); buttonID++) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
        if (prevMenuButtonID == MENU_BUTTON_ERASE     ) for ((buttonID = MENU_BUTTON_ERASE_MIN ); (buttonID < MENU_BUTTON_ERASE_MAX ); buttonID++) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
...
void load_score_menu_from_submenu(s16 prevMenuButtonID, struct Object *sourceButton) {
    ...
    if (sMainMenuButtons[prevMenuButtonID]->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        // Hide buttons of corresponding button menu groups
        // if (prevMenuButtonID == MENU_BUTTON_SCORE) for ((buttonID = MENU_BUTTON_SCORE_MIN); (buttonID < MENU_BUTTON_SCORE_MAX); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]); // Not possible, this is checking if the score menu was opened from the score menu!
        if (prevMenuButtonID == MENU_BUTTON_COPY ) for ((buttonID = MENU_BUTTON_COPY_MIN ); (buttonID < MENU_BUTTON_COPY_MAX ); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
        if (prevMenuButtonID == MENU_BUTTON_ERASE) for ((buttonID = MENU_BUTTON_ERASE_MIN); (buttonID < MENU_BUTTON_ERASE_MAX); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
...
void load_copy_menu_from_submenu(s16 prevMenuButtonID, struct Object *sourceButton) {
    ...
    if (sMainMenuButtons[prevMenuButtonID]->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        // Hide buttons of corresponding button menu groups
        if (prevMenuButtonID == MENU_BUTTON_SCORE) for ((buttonID = MENU_BUTTON_SCORE_MIN); (buttonID < MENU_BUTTON_SCORE_MAX); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
        // if (prevMenuButtonID == MENU_BUTTON_COPY ) for ((buttonID = MENU_BUTTON_COPY_MIN ); (buttonID < MENU_BUTTON_COPY_MAX ); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]); // Not possible, this is checking if the copy menu was opened from the copy menu!
        if (prevMenuButtonID == MENU_BUTTON_ERASE) for ((buttonID = MENU_BUTTON_ERASE_MIN); (buttonID < MENU_BUTTON_ERASE_MAX); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
...
void load_erase_menu_from_submenu(s16 prevMenuButtonID, struct Object *sourceButton) {
    ...
    if (sMainMenuButtons[prevMenuButtonID]->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        ...
        if (prevMenuButtonID == MENU_BUTTON_SCORE) for ((buttonID = MENU_BUTTON_SCORE_MIN); (buttonID < MENU_BUTTON_SCORE_MAX); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
        if (prevMenuButtonID == MENU_BUTTON_COPY ) for ((buttonID = MENU_BUTTON_COPY_MIN ); (buttonID < MENU_BUTTON_COPY_MAX ); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]);
        // if (prevMenuButtonID == MENU_BUTTON_ERASE) for ((buttonID = MENU_BUTTON_ERASE_MIN); (buttonID < MENU_BUTTON_ERASE_MAX); (buttonID++)) obj_mark_for_deletion(sMainMenuButtons[buttonID]); // Not possible, this is checking if the erase menu was opened from the erase menu!
...
void bhv_menu_button_manager_init(void) {
    // File A
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_A] = spawn_object_rel_with_rot(o, (save_file_exists(SAVE_FILE_A) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE), bhvMenuButton, -6400, 2800, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_A]->oMenuButtonScale = 1.0f;
    // File B
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_B] = spawn_object_rel_with_rot(o, (save_file_exists(SAVE_FILE_B) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE), bhvMenuButton,  1500, 2800, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_B]->oMenuButtonScale = 1.0f;
    // File C
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_C] = spawn_object_rel_with_rot(o, (save_file_exists(SAVE_FILE_C) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE), bhvMenuButton, -6400,    0, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_C]->oMenuButtonScale = 1.0f;
    // File D
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_D] = spawn_object_rel_with_rot(o, (save_file_exists(SAVE_FILE_D) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE), bhvMenuButton,  1500,    0, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_D]->oMenuButtonScale = 1.0f;

    // Score menu button
    sMainMenuButtons[MENU_BUTTON_SCORE      ] = spawn_object_rel_with_rot(o, MODEL_MAIN_MENU_GREEN_SCORE_BUTTON,  bhvMenuButton, -6400, -3500, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_SCORE      ]->oMenuButtonScale = 1.0f;
    // Copy menu button
    sMainMenuButtons[MENU_BUTTON_COPY       ] = spawn_object_rel_with_rot(o, MODEL_MAIN_MENU_BLUE_COPY_BUTTON,    bhvMenuButton, -2134, -3500, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_COPY       ]->oMenuButtonScale = 1.0f;
    // Erase menu button
    sMainMenuButtons[MENU_BUTTON_ERASE      ] = spawn_object_rel_with_rot(o, MODEL_MAIN_MENU_RED_ERASE_BUTTON,    bhvMenuButton,  2134, -3500, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_ERASE      ]->oMenuButtonScale = 1.0f;
    // Sound mode menu button (Option Mode in EU)
    sMainMenuButtons[MENU_BUTTON_SOUND_MODE ] = spawn_object_rel_with_rot(o, MODEL_MAIN_MENU_PURPLE_SOUND_BUTTON, bhvMenuButton,  6400, -3500, 0, 0x0, 0x0, 0x0);
