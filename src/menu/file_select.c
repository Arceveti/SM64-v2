...
uchar textEraseFileButton[] = { TEXT_ERASE_FILE_BUTTON };

uchar textSoundModes[][8] = { { TEXT_STEREO }, { TEXT_MONO }, { TEXT_HEADSET } };
...
uchar textMarioC[] = { TEXT_FILE_MARIO_C };
uchar textMarioD[] = { TEXT_FILE_MARIO_D };

uchar textNew[]  = { TEXT_NEW };
uchar starIcon[] = { GLYPH_STAR, GLYPH_SPACE };
uchar xIcon[]    = { GLYPH_MULTIPLY, GLYPH_SPACE };

uchar textSelectFile[]            = { TEXT_SELECT_FILE };
uchar textScore[]                 = { TEXT_SCORE };
uchar textCopy[]                  = { TEXT_COPY  };
uchar textErase[]                 = { TEXT_ERASE };
uchar textLanguage[][9]           = {{ TEXT_ENGLISH }, { TEXT_FRENCH }, { TEXT_GERMAN }};
uchar textCheckFile[]             = { TEXT_CHECK_FILE };
uchar textNoSavedDataExists[]     = { TEXT_NO_SAVED_DATA_EXISTS };
uchar textCopyFile[]              = { TEXT_COPY_FILE };
uchar textCopyItToWhere[]         = { TEXT_COPY_IT_TO_WHERE };
uchar textNoSavedDataExistsCopy[] = { TEXT_NO_SAVED_DATA_EXISTS };
uchar textCopyCompleted[]         = { TEXT_COPYING_COMPLETED };
uchar textSavedDataExists[]       = { TEXT_SAVED_DATA_EXISTS };
uchar textNoFileToCopyFrom[]      = { TEXT_NO_FILE_TO_COPY_FROM };
uchar textYes[]                   = { TEXT_YES };
uchar textNo[]                    = { TEXT_NO };
...
void beh_yellow_background_menu_init(void) {
    gCurrentObject->oFaceAngleYaw    = DEGREES(180);
...
void bhv_menu_button_zoom_in_out(struct Object *button) {
    if (sCurrentMenuLevel == MENU_LAYER_MAIN) {
        if (button->oMenuButtonTimer <  4) button->oParentRelativePosZ -= 20.0f;
        if (button->oMenuButtonTimer >= 4) button->oParentRelativePosZ += 20.0f;
    } else {
        if (button->oMenuButtonTimer <  4) button->oParentRelativePosZ += 20.0f;
        if (button->oMenuButtonTimer >= 4) button->oParentRelativePosZ -= 20.0f;
...
void bhv_menu_button_loop(void) {
    switch (gCurrentObject->oMenuButtonState) {
        ...
        case MENU_BUTTON_STATE_GROWING: // Switching from button to menu state
            if (sCurrentMenuLevel == MENU_LAYER_MAIN   ) bhv_menu_button_growing_from_main_menu(gCurrentObject);
            if (sCurrentMenuLevel == MENU_LAYER_SUBMENU) bhv_menu_button_growing_from_submenu(  gCurrentObject); // Only used for score files
            sTextBaseAlpha       = 0;
        ...
        case MENU_BUTTON_STATE_SHRINKING: // Switching from menu to button state
            if (sCurrentMenuLevel == MENU_LAYER_MAIN   ) bhv_menu_button_shrinking_to_main_menu(gCurrentObject);
            if (sCurrentMenuLevel == MENU_LAYER_SUBMENU) bhv_menu_button_shrinking_to_submenu(  gCurrentObject); // Only used for score files
            sTextBaseAlpha       = 0;
            sCursorClickingTimer = 4;
            break;
...
void exit_score_file_to_score_menu(struct Object *scoreFileButton, s8 scoreButtonID) {
    ...
    if (scoreFileButton->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        ...
        if (sCurrentMenuLevel == MENU_LAYER_SUBMENU) sCurrentMenuLevel = MENU_LAYER_MAIN;
    }
}
...
void render_score_menu_buttons(struct Object *scoreButton) {
    // File A
    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_A    ] = spawn_object_rel_with_rot(scoreButton, save_file_exists(SAVE_FILE_A) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton,  711, 311, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_A    ]->oMenuButtonScale = 0.11111111f;
    // File B
    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_B    ] = spawn_object_rel_with_rot(scoreButton, save_file_exists(SAVE_FILE_B) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166, 311, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_B    ]->oMenuButtonScale = 0.11111111f;
    // File C
    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_C    ] = spawn_object_rel_with_rot(scoreButton, save_file_exists(SAVE_FILE_C) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton,  711,   0, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_C    ]->oMenuButtonScale = 0.11111111f;
    // File D
    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_D    ] = spawn_object_rel_with_rot(scoreButton, save_file_exists(SAVE_FILE_D) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166,   0, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_SCORE_FILE_D    ]->oMenuButtonScale = 0.11111111f;
    // Return to main menu button
    sMainMenuButtons[MENU_BUTTON_SCORE_RETURN    ] = spawn_object_rel_with_rot(scoreButton, MODEL_MAIN_MENU_YELLOW_FILE_BUTTON, bhvMenuButton,  711, -388, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_SCORE_RETURN    ]->oMenuButtonScale = 0.11111111f;
    // Switch to copy menu button
    sMainMenuButtons[MENU_BUTTON_SCORE_COPY_FILE ] = spawn_object_rel_with_rot(scoreButton, MODEL_MAIN_MENU_BLUE_COPY_BUTTON,   bhvMenuButton,    0, -388, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_SCORE_COPY_FILE ]->oMenuButtonScale = 0.11111111f;
    // Switch to erase menu button
    sMainMenuButtons[MENU_BUTTON_SCORE_ERASE_FILE] = spawn_object_rel_with_rot(scoreButton, MODEL_MAIN_MENU_RED_ERASE_BUTTON,   bhvMenuButton, -711, -388, -100, 0x0, -DEGREES(180), 0x0);
...
void render_copy_menu_buttons(struct Object *copyButton) {
    // File A
    sMainMenuButtons[MENU_BUTTON_COPY_FILE_A     ] = spawn_object_rel_with_rot(copyButton, save_file_exists(SAVE_FILE_A) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton,  711, 311, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_COPY_FILE_A     ]->oMenuButtonScale = 0.11111111f;
    // File B
    sMainMenuButtons[MENU_BUTTON_COPY_FILE_B     ] = spawn_object_rel_with_rot(copyButton, save_file_exists(SAVE_FILE_B) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166, 311, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_COPY_FILE_B     ]->oMenuButtonScale = 0.11111111f;
    // File C
    sMainMenuButtons[MENU_BUTTON_COPY_FILE_C     ] = spawn_object_rel_with_rot(copyButton, save_file_exists(SAVE_FILE_C) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton,  711,   0, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_COPY_FILE_C     ]->oMenuButtonScale = 0.11111111f;
    // File D
    sMainMenuButtons[MENU_BUTTON_COPY_FILE_D     ] = spawn_object_rel_with_rot(copyButton, save_file_exists(SAVE_FILE_D) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166,   0, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_COPY_FILE_D     ]->oMenuButtonScale = 0.11111111f;
    // Return to main menu button
    sMainMenuButtons[MENU_BUTTON_COPY_RETURN     ] = spawn_object_rel_with_rot(copyButton, MODEL_MAIN_MENU_YELLOW_FILE_BUTTON, bhvMenuButton,  711, -388, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_COPY_RETURN     ]->oMenuButtonScale = 0.11111111f;
    // Switch to scire menu button
    sMainMenuButtons[MENU_BUTTON_COPY_CHECK_SCORE] = spawn_object_rel_with_rot(copyButton, MODEL_MAIN_MENU_GREEN_SCORE_BUTTON, bhvMenuButton,    0, -388, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_COPY_CHECK_SCORE]->oMenuButtonScale = 0.11111111f;
    // Switch to erase menu button
    sMainMenuButtons[MENU_BUTTON_COPY_ERASE_FILE ] = spawn_object_rel_with_rot(copyButton, MODEL_MAIN_MENU_RED_ERASE_BUTTON,   bhvMenuButton, -711, -388, -100, 0x0, -DEGREES(180), 0x0);
...
void render_erase_menu_buttons(struct Object *eraseButton) {
    // File A
    sMainMenuButtons[MENU_BUTTON_ERASE_FILE_A     ] = spawn_object_rel_with_rot(eraseButton, save_file_exists(SAVE_FILE_A) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton,  711, 311, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_ERASE_FILE_A     ]->oMenuButtonScale = 0.11111111f;
    // File B
    sMainMenuButtons[MENU_BUTTON_ERASE_FILE_B     ] = spawn_object_rel_with_rot(eraseButton, save_file_exists(SAVE_FILE_B) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166, 311, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_ERASE_FILE_B     ]->oMenuButtonScale = 0.11111111f;
    // File C
    sMainMenuButtons[MENU_BUTTON_ERASE_FILE_C     ] = spawn_object_rel_with_rot(eraseButton, save_file_exists(SAVE_FILE_C) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton,  711,   0, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_ERASE_FILE_C     ]->oMenuButtonScale = 0.11111111f;
    // File D
    sMainMenuButtons[MENU_BUTTON_ERASE_FILE_D     ] = spawn_object_rel_with_rot(eraseButton, save_file_exists(SAVE_FILE_D) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON : MODEL_MAIN_MENU_MARIO_NEW_BUTTON, bhvMenuButton, -166,   0, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_ERASE_FILE_D     ]->oMenuButtonScale = 0.11111111f;
    // Return to main menu button
    sMainMenuButtons[MENU_BUTTON_ERASE_RETURN     ] = spawn_object_rel_with_rot(eraseButton, MODEL_MAIN_MENU_YELLOW_FILE_BUTTON, bhvMenuButton,  711, -388, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_ERASE_RETURN     ]->oMenuButtonScale = 0.11111111f;
    // Switch to score menu button
    sMainMenuButtons[MENU_BUTTON_ERASE_CHECK_SCORE] = spawn_object_rel_with_rot(eraseButton, MODEL_MAIN_MENU_GREEN_SCORE_BUTTON, bhvMenuButton,    0, -388, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_ERASE_CHECK_SCORE]->oMenuButtonScale = 0.11111111f;
    // Switch to copy menu button
    sMainMenuButtons[MENU_BUTTON_ERASE_COPY_FILE  ] = spawn_object_rel_with_rot(eraseButton, MODEL_MAIN_MENU_BLUE_COPY_BUTTON,   bhvMenuButton, -711, -388, -100, 0x0, -DEGREES(180), 0x0);
        ...
        for ((buttonID = MENU_BUTTON_ERASE_MIN); (buttonID < MENU_BUTTON_ERASE_MAX); (buttonID++)) {
            ...
            if (check_clicked_button(buttonX, buttonY, 22.0f)) {
                ...
                if ((buttonID == MENU_BUTTON_ERASE_RETURN) || (buttonID == MENU_BUTTON_ERASE_CHECK_SCORE) || (buttonID == MENU_BUTTON_ERASE_COPY_FILE)) {
                    ...
                    if (sMainMenuTimer >= ACTION_TIMER) erase_action_file_button(eraseButton, buttonID);
                }
                sCurrentMenuLevel = MENU_LAYER_SUBMENU;
                break;
            }
...
void render_sound_mode_menu_buttons(struct Object *soundModeButton) {
    ...
    sMainMenuButtons[MENU_BUTTON_STEREO ] = spawn_object_rel_with_rot(soundModeButton, MODEL_MAIN_MENU_GENERIC_BUTTON, bhvMenuButton,  533, SOUND_BUTTON_Y, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_STEREO ]->oMenuButtonScale = 0.11111111f;
    // Mono option button
    sMainMenuButtons[MENU_BUTTON_MONO   ] = spawn_object_rel_with_rot(soundModeButton, MODEL_MAIN_MENU_GENERIC_BUTTON, bhvMenuButton,    0, SOUND_BUTTON_Y, -100, 0x0, -DEGREES(180), 0x0);
    sMainMenuButtons[MENU_BUTTON_MONO   ]->oMenuButtonScale = 0.11111111f;
    // Headset option button
    sMainMenuButtons[MENU_BUTTON_HEADSET] = spawn_object_rel_with_rot(soundModeButton, MODEL_MAIN_MENU_GENERIC_BUTTON, bhvMenuButton, -533, SOUND_BUTTON_Y, -100, 0x0, -DEGREES(180), 0x0);
    ...
    sMainMenuButtons[MENU_BUTTON_LANGUAGE_ENGLISH] = spawn_object_rel_with_rot(
        soundModeButton, MODEL_MAIN_MENU_GENERIC_BUTTON, bhvMenuButton, 533, -111, -100, 0, -DEGREES(180), 0);
    ...
    sMainMenuButtons[MENU_BUTTON_LANGUAGE_FRENCH] = spawn_object_rel_with_rot(
        soundModeButton, MODEL_MAIN_MENU_GENERIC_BUTTON, bhvMenuButton,   0, -111, -100, 0, -DEGREES(180), 0);
    ...
    sMainMenuButtons[MENU_BUTTON_LANGUAGE_RETURN] = spawn_object_rel_with_rot(
        soundModeButton, MODEL_MAIN_MENU_YELLOW_FILE_BUTTON, bhvMenuButton, 0, -533, -100, 0, -DEGREES(180), 0);
    ...
    if (soundModeButton->oMenuButtonState == MENU_BUTTON_STATE_FULLSCREEN) {
        ...
        for ((buttonID = MENU_BUTTON_OPTION_MIN); (buttonID < MENU_BUTTON_OPTION_MAX); (buttonID++)) {
            ...
            if (check_clicked_button(buttonX, buttonY, 22.0f)) {
                ...
                if ((buttonID == MENU_BUTTON_LANGUAGE_ENGLISH) || (buttonID == MENU_BUTTON_LANGUAGE_FRENCH) || (buttonID == MENU_BUTTON_LANGUAGE_GERMAN)) {
                    if (soundModeButton->oMenuButtonActionPhase == SOUND_MODE_PHASE_MAIN) {
                        play_sound(SOUND_MENU_CLICK_FILE_SELECT, gGlobalSoundSource);
                        sMainMenuButtons[buttonID]->oMenuButtonState = MENU_BUTTON_STATE_ZOOM_IN_OUT;
...
void load_main_menu_save_file(struct Object *fileButton, s32 fileNum) {
    if (fileButton->oMenuButtonState == MENU_BUTTON_STATE_FULLSCREEN) sSelectedFileNum = fileNum;
}
...
void return_to_main_menu(s16 prevMenuButtonID, struct Object *sourceButton) {
    ...
    if (sMainMenuButtons[prevMenuButtonID]->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        ...
        if (prevMenuButtonID == MENU_BUTTON_SCORE     ) for ((buttonID = MENU_BUTTON_SCORE_MIN ); (buttonID < MENU_BUTTON_SCORE_MAX ); buttonID++) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
        if (prevMenuButtonID == MENU_BUTTON_COPY      ) for ((buttonID = MENU_BUTTON_COPY_MIN  ); (buttonID < MENU_BUTTON_COPY_MAX  ); buttonID++) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
        if (prevMenuButtonID == MENU_BUTTON_ERASE     ) for ((buttonID = MENU_BUTTON_ERASE_MIN ); (buttonID < MENU_BUTTON_ERASE_MAX ); buttonID++) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
...
void load_score_menu_from_submenu(s16 prevMenuButtonID, struct Object *sourceButton) {
    ...
    if (sMainMenuButtons[prevMenuButtonID]->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        // Hide buttons of corresponding button menu groups
        // if (prevMenuButtonID == MENU_BUTTON_SCORE) for ((buttonID = MENU_BUTTON_SCORE_MIN); (buttonID < MENU_BUTTON_SCORE_MAX); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]); //! Not possible, this is checking if the score menu was opened from the score menu!
        if (prevMenuButtonID == MENU_BUTTON_COPY ) for ((buttonID = MENU_BUTTON_COPY_MIN ); (buttonID < MENU_BUTTON_COPY_MAX ); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
        if (prevMenuButtonID == MENU_BUTTON_ERASE) for ((buttonID = MENU_BUTTON_ERASE_MIN); (buttonID < MENU_BUTTON_ERASE_MAX); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
...
void load_copy_menu_from_submenu(s16 prevMenuButtonID, struct Object *sourceButton) {
    ...
    if (sMainMenuButtons[prevMenuButtonID]->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        // Hide buttons of corresponding button menu groups
        if (prevMenuButtonID == MENU_BUTTON_SCORE) for ((buttonID = MENU_BUTTON_SCORE_MIN); (buttonID < MENU_BUTTON_SCORE_MAX); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
        // if (prevMenuButtonID == MENU_BUTTON_COPY ) for ((buttonID = MENU_BUTTON_COPY_MIN ); (buttonID < MENU_BUTTON_COPY_MAX ); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]); //! Not possible, this is checking if the copy menu was opened from the copy menu!
        if (prevMenuButtonID == MENU_BUTTON_ERASE) for ((buttonID = MENU_BUTTON_ERASE_MIN); (buttonID < MENU_BUTTON_ERASE_MAX); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
...
void load_erase_menu_from_submenu(s16 prevMenuButtonID, struct Object *sourceButton) {
    ...
    if (sMainMenuButtons[prevMenuButtonID]->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
        ...
        if (prevMenuButtonID == MENU_BUTTON_SCORE) for ((buttonID = MENU_BUTTON_SCORE_MIN); (buttonID < MENU_BUTTON_SCORE_MAX); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
        if (prevMenuButtonID == MENU_BUTTON_COPY ) for ((buttonID = MENU_BUTTON_COPY_MIN ); (buttonID < MENU_BUTTON_COPY_MAX ); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]);
        // if (prevMenuButtonID == MENU_BUTTON_ERASE) for ((buttonID = MENU_BUTTON_ERASE_MIN); (buttonID < MENU_BUTTON_ERASE_MAX); (buttonID++)) mark_obj_for_deletion(sMainMenuButtons[buttonID]); //! Not possible, this is checking if the erase menu was opened from the erase menu!
...
void bhv_menu_button_manager_init(void) {
    // File A
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_A] = spawn_object_rel_with_rot(gCurrentObject, (save_file_exists(SAVE_FILE_A) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE), bhvMenuButton, -6400, 2800, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_A]->oMenuButtonScale = 1.0f;
    // File B
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_B] = spawn_object_rel_with_rot(gCurrentObject, (save_file_exists(SAVE_FILE_B) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE), bhvMenuButton,  1500, 2800, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_B]->oMenuButtonScale = 1.0f;
    // File C
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_C] = spawn_object_rel_with_rot(gCurrentObject, (save_file_exists(SAVE_FILE_C) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE), bhvMenuButton, -6400,    0, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_C]->oMenuButtonScale = 1.0f;
    // File D
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_D] = spawn_object_rel_with_rot(gCurrentObject, (save_file_exists(SAVE_FILE_D) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE), bhvMenuButton,  1500,    0, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_PLAY_FILE_D]->oMenuButtonScale = 1.0f;

    // Score menu button
    sMainMenuButtons[MENU_BUTTON_SCORE      ] = spawn_object_rel_with_rot(gCurrentObject, MODEL_MAIN_MENU_GREEN_SCORE_BUTTON,  bhvMenuButton, -6400, -3500, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_SCORE      ]->oMenuButtonScale = 1.0f;
    // Copy menu button
    sMainMenuButtons[MENU_BUTTON_COPY       ] = spawn_object_rel_with_rot(gCurrentObject, MODEL_MAIN_MENU_BLUE_COPY_BUTTON,    bhvMenuButton, -2134, -3500, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_COPY       ]->oMenuButtonScale = 1.0f;
    // Erase menu button
    sMainMenuButtons[MENU_BUTTON_ERASE      ] = spawn_object_rel_with_rot(gCurrentObject, MODEL_MAIN_MENU_RED_ERASE_BUTTON,    bhvMenuButton,  2134, -3500, 0, 0x0, 0x0, 0x0);
    sMainMenuButtons[MENU_BUTTON_ERASE      ]->oMenuButtonScale = 1.0f;
    // Sound mode menu button (Option Mode in EU)
    sMainMenuButtons[MENU_BUTTON_SOUND_MODE ] = spawn_object_rel_with_rot(gCurrentObject, MODEL_MAIN_MENU_PURPLE_SOUND_BUTTON, bhvMenuButton,  6400, -3500, 0, 0x0, 0x0, 0x0);
...
void print_menu_cursor(void) {
    ...
    if (sCursorClickingTimer == 0) gSPDisplayList(gDisplayListHead++, dl_menu_idle_hand); // Idle
...
s32 update_text_fade_out(void) {
    if (sFadeOutText) {
        ...
        if (sTextFadeAlpha > 0) sTextFadeAlpha -= 50;
    }
    return FALSE;
}
...
void copy_menu_display_message(s8 messageID) {
    switch (messageID) {
        ...
        case COPY_MSG_NOSAVE_EXISTS: print_generic_string_fade(NOSAVE_DATA_X2, 190, textNoSavedDataExistsCopy          ); break;
        case COPY_MSG_COPY_COMPLETE: print_generic_string_fade(COPYCOMPLETE_X, 190, LANGUAGE_ARRAY(textCopyCompleted)  ); break;
        case COPY_MSG_SAVE_EXISTS:   print_generic_string_fade(SAVE_EXISTS_X1, 190, LANGUAGE_ARRAY(textSavedDataExists)); break;
    }
}
...
void erase_menu_update_message(void) {
    switch (sMainMenuButtons[MENU_BUTTON_ERASE]->oMenuButtonActionPhase) {
        case ERASE_PHASE_MAIN:
            if ((sMainMenuTimer == FADEOUT_TIMER) && (sStatusMessageID == ERASE_MSG_NOSAVE_EXISTS)) sFadeOutText = TRUE;
            if (update_text_fade_out()) sStatusMessageID = ((sStatusMessageID == ERASE_MSG_MAIN_TEXT) ? ERASE_MSG_NOSAVE_EXISTS : ERASE_MSG_MAIN_TEXT);
            break;
        case ERASE_PHASE_PROMPT:
            if (update_text_fade_out()) {
                if (sStatusMessageID != ERASE_MSG_PROMPT) sStatusMessageID = ERASE_MSG_PROMPT;
                sCursorPos[0] = 43.0f;
                sCursorPos[1] = 80.0f;
            }
            break;
        case ERASE_PHASE_MARIO_ERASED:
            if (sMainMenuTimer == FADEOUT_TIMER) sFadeOutText = TRUE;
            if (update_text_fade_out()) sStatusMessageID = ((sStatusMessageID != ERASE_MSG_MARIO_ERASED) ? ERASE_MSG_MARIO_ERASED : ERASE_MSG_MAIN_TEXT);
            break;
    }
}
...
void print_file_select_strings(void) {
    ...
    if (sTextBaseAlpha <  250) sTextBaseAlpha += 10;
    if (sMainMenuTimer < 1000) sMainMenuTimer++;
}
