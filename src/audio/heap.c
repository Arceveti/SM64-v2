...
f64 root_newton_step(f64 x, s32 k, f64 d) {
    ...
    f64 fx;
    f64 deriv = deg8;
    if (degree & 0x1) deriv *= x;
    if (degree & 0x2) deriv *= deg2;
    if (degree & 0x4) deriv *= deg4;
    if (degree & 0x8) deriv *= deg8;
    fx    = ((deriv * x) - d);
    deriv = (k * deriv);
    return (x - (fx / deriv));
}
...
#ifdef VERSION_SH
    for ((i = 0); (i <  64); (i++)) if (gBankLoadStatus[i] != SOUND_LOAD_STATUS_5) gBankLoadStatus[i] = SOUND_LOAD_STATUS_NOT_LOADED;
    for ((i = 0); (i <  64); (i++)) if (gUnkLoadStatus [i] != SOUND_LOAD_STATUS_5) gUnkLoadStatus [i] = SOUND_LOAD_STATUS_NOT_LOADED;
    for ((i = 0); (i < 256); (i++)) if (gSeqLoadStatus [i] != SOUND_LOAD_STATUS_5) gSeqLoadStatus [i] = SOUND_LOAD_STATUS_NOT_LOADED;
#else
    for ((i = 0); (i <  64); (i++)) gBankLoadStatus[i] = SOUND_LOAD_STATUS_NOT_LOADED;
...
void *soundAlloc(struct SoundAllocPool *pool, u32 size) {
    ...
    if ((start + alignedSize) <= (pool->start + pool->size)) {
        ...
        for ((pos = start); (pos < pool->cur); (pos++)) *pos = 0;
    } else {
...
#define isSound poolIdx
    ...
    u8 isSound = FALSE;
#endif
    u16 firstVal, secondVal;
#if defined(VERSION_JP) || defined(VERSION_US)
    u32 bothDiscardable, leftDiscardable, rightDiscardable;
...
#ifdef VERSION_SH
    switch (poolIdx) {
        case 0: arg0 =    &gSeqLoadedPool; table =  gSeqLoadStatus; break;
        case 1: arg0 =   &gBankLoadedPool; table = gBankLoadStatus; break;
        case 2: arg0 = &gUnusedLoadedPool; table =  gUnkLoadStatus; break;
    }
#endif
    if (arg3 == 0) {
        tp = &arg0->temporary;
        ...
        if (poolIdx == 1) {
            if (firstVal == SOUND_LOAD_STATUS_4) {
                for ((i = 0); (i < gMaxSimultaneousNotes); (i++)) if ((gNotes[i].bankId == tp->entries[0].id) && gNotes[i].noteSubEu.enabled) break;
            ...
            if (secondVal == SOUND_LOAD_STATUS_4) {
                for ((i = 0); (i < gMaxSimultaneousNotes); (i++)) if ((gNotes[i].bankId == tp->entries[1].id) && gNotes[i].noteSubEu.enabled) break;
                if (i == gMaxSimultaneousNotes) {
                    if (gBankLoadStatus[tp->entries[1].id] != SOUND_LOAD_STATUS_5) gBankLoadStatus[tp->entries[1].id] = SOUND_LOAD_STATUS_DISCARDABLE;
            ...
            if ((firstVal == SOUND_LOAD_STATUS_DISCARDABLE) && (secondVal == SOUND_LOAD_STATUS_DISCARDABLE)) {
                ...
                if (poolIdx == 0) {
                    if (firstVal == SOUND_LOAD_STATUS_COMPLETE) {
                        for ((i = 0); (i < SEQUENCE_PLAYERS); (i++)) if (gSequencePlayers[i].enabled && (gSequencePlayers[i].seqId == tp->entries[0].id)) break;
                    ...
                    if (secondVal == SOUND_LOAD_STATUS_COMPLETE) {
                        for (i = 0; i < SEQUENCE_PLAYERS; i++) if (gSequencePlayers[i].enabled && gSequencePlayers[i].seqId == tp->entries[1].id) break;
                        if (i == SEQUENCE_PLAYERS) {
                            tp->nextSide = 1;
                            goto out;
                        }
        ...
        if (tp->entries[tp->nextSide].id != (s8)nullID) {
            ...
            if (isSound == TRUE) discard_bank(tp->entries[tp->nextSide].id);
        }
        switch (tp->nextSide) {
            case 0:
...
void *get_bank_or_seq_inner(s32 poolIdx, s32 arg1, s32 bankId) {
    ...
    switch (poolIdx) {
        case 0: loadedPool = &gSeqLoadedPool;    break;
        case 1: loadedPool = &gBankLoadedPool;   break;
        case 2: loadedPool = &gUnusedLoadedPool; break;
    }
    ...
    for ((i = 0); (i < persistent->numEntries); (i++)) if (persistent->entries[i].id == bankId) return persistent->entries[i].ptr;
    if (arg1 == 2) return get_bank_or_seq(poolIdx, 0, bankId);
    return NULL;
}
#endif
...
void *get_bank_or_seq(struct SoundMultiPool *arg0, s32 arg1, s32 id) {
    ...
    if (arg1 == 0) {
        ...
        if (arg1 == 2) return get_bank_or_seq(arg0, 0, id);
        return NULL;
    }
}
#endif
...
void decrease_reverb_gain(void) {
#if defined(VERSION_EU)
    s32 i;
    for ((i = 0); (i < gNumSynthesisReverbs); (i++)) gSynthesisReverbs[i].reverbGain -= (gSynthesisReverbs[i].reverbGain / 8);
    ...
    for ((i = 0); (i < gNumSynthesisReverbs); (i++)) for ((j = 0); (j < v0); (j++)) gSynthesisReverbs[i].reverbGain -= (gSynthesisReverbs[i].reverbGain / 3);
#endif
}
...
void clear_curr_ai_buffer(void) {
    ...
    for ((i = 0); (i < (s32) (AIBUFFER_LEN / sizeof(s16))); (i++)) gAiBuffers[currIndex][i] = 0;
}
#endif
...
s32 audio_shut_down_and_reset_step(void) {
    ...
    if (gAudioResetStatus < 3) return 0;
    return 1;
}
#else
/**
...
void audio_reset_session(void) {
    ...
    if (gAudioLoadLock != AUDIO_LOCK_UNINITIALIZED) {
        ...
        for (;;) {
            ...
            frames++;
            if (frames > (4 * 60)) break; // Break after 4 seconds
            for ((i = 0); (i < gMaxSimultaneousNotes); (i++)) if (gNotes[i].enabled) break;
            if (i == gMaxSimultaneousNotes) break; // All zero, break early
        }
        ...
        while (remainingDmas > 0) for ((i = 0); (i < gCurrAudioFrameDmaCount); (i++)) if (osRecvMesg(&gCurrAudioFrameDmaQueue, NULL, OS_MESG_NOBLOCK) == 0) remainingDmas--;
        gCurrAudioFrameDmaCount = 0;
        for ((j = 0); (j < NUMAIBUFFERS); (j++)) for ((k = 0); (k < (s32) (AIBUFFER_LEN / sizeof(s16))); (k++)) gAiBuffers[j][k] = 0;
    }
#endif
...
#ifdef VERSION_SH
    if (gAudioBufferParameters.presetUnk4 >= 2) gAudioBufferParameters.maxAiBufferLength -= 0x10;
    ...
    reverbConsole = (gIsConsole ? betterReverbDownsampleConsole : betterReverbDownsampleEmulator); // Setting this to 1 is REALLY slow, please use sparingly!
    if (reverbConsole <= 0) {
        reverbConsole      = 1;
    ...
    if (toggleBetterReverb && (betterReverbWindowsSize >= 0)) reverbWindowSize = betterReverbWindowsSize;
    if (gReverbDownsampleRate < (1 << (reverbConsole - 1))) gReverbDownsampleRate = (1 << (reverbConsole - 1));
    reverbWindowSize /= gReverbDownsampleRate;
    if (reverbWindowSize < DEFAULT_LEN_2CH) reverbWindowSize = DEFAULT_LEN_2CH; // Minimum window size to not overflow
#endif
    gVolume               = preset->volume;
...
#if defined(VERSION_JP) || defined(VERSION_US)
    for ((j = 0); (j < 2); (j++)) gAudioCmdBuffers[j] = soundAlloc(&gNotesAndBuffersPool, (gMaxAudioCmds * sizeof(u64)));
#endif
    ...
    for ((j = 0); (j != 2); (j++)) gAudioCmdBuffers[j] = soundAlloc(&gNotesAndBuffersPool, (gMaxAudioCmds * sizeof(u64)));
    for ((j = 0); (j <  4); (j++)) gSynthesisReverbs[j].useReverb = 0;
...
#if defined(VERSION_JP) || defined(VERSION_US)
    if (gAudioLoadLock != AUDIO_LOCK_UNINITIALIZED) gAudioLoadLock = AUDIO_LOCK_NOT_LOADING;
#endif
}
...
void *unk_pool1_lookup(s32 poolIdx, s32 id) {
    s32 i;
    for ((i = 0); (i < gUnkPool1.pool.numAllocatedEntries); (i++)) if ((gUnkPool1.entries[i].poolIndex == poolIdx) && (gUnkPool1.entries[i].id == id)) return gUnkPool1.entries[i].ptr;
    return NULL;
}
...
struct UnkEntry *func_sh_802f1ec4(u32 size) {
    ...
    if (chosenIndex == -1) chosenIndex = pool->numEntries++;
...
void func_sh_802f2158(struct UnkEntry *entry) {
    ...
    for ((idx = 0); (idx < seqCount); (idx++)) {
        ...
        if ((((bankId1 != 0xff) && (entry->bankId == bankId1)) || ((bankId2 != 0xff) && (entry->bankId == bankId2)) || (entry->bankId == 0))
            && (get_bank_or_seq(1, 2, idx) != NULL)
            && (IS_BANK_LOAD_COMPLETE(idx))) {
            for ((instId = 0); (instId < gCtlEntries[idx].numInstruments); (instId++)) {
                inst = get_instrument_inner(idx, instId);
                if (inst != NULL) {
                    if (inst->normalRangeLo !=   0) func_sh_802F2320(entry, inst->lowNotesSound.sample );
                    if (inst->normalRangeHi != 127) func_sh_802F2320(entry, inst->highNotesSound.sample);
            ...
            for ((drumId = 0); (drumId < gCtlEntries[idx].numDrums); (drumId++)) {
                ...
                if (drum != NULL) func_sh_802F2320(entry, drum->sound.sample);
            }
        }
    }
}
...
void func_sh_802f23ec(void) {
    ...
    for ((idx = 0); (idx < seqCount); (idx++)) {
        bankId1 = gCtlEntries[idx].bankId1;
        bankId2 = gCtlEntries[idx].bankId2;
        if ((((bankId1 != 0xffu) && (entry->bankId == bankId1)) || ((bankId2 != 0xff) && (entry->bankId == bankId2)) || (entry->bankId == 0)) && (get_bank_or_seq(1, 3, idx) != NULL) && IS_BANK_LOAD_COMPLETE(idx)) {
