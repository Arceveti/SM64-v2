...
void init_reverb_eu(void) {
    ...
    for ((j = 0); (j < 4); (j++)) {
        ...
        if (!sAudioFirstBoot) gSynthesisReverbs[j].ringBuffer.left = soundAlloc(&gNotesAndBuffersPool, (REVERB_WINDOW_SIZE_MAX * 4));
    }
    ...
    for ((j = 0); (j < gNumSynthesisReverbs); (j++)) {
        ...
        reverb->useReverb      = 8;
        if (reverb->windowSize > REVERB_WINDOW_SIZE_MAX) reverb->windowSize = REVERB_WINDOW_SIZE_MAX;
        if (sAudioFirstBoot) {
...
void init_reverb_us(s32 presetId) {
    ...
    if (reverbWindowSize == 0) {
        ...
        if (reverbWindowSize > REVERB_WINDOW_SIZE_MAX) reverbWindowSize = REVERB_WINDOW_SIZE_MAX;
        // Both left and right channels are allocated/cleared together, then separated based on the reverb window size
        if (!sAudioFirstBoot) {
