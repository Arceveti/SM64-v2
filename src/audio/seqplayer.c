...
void seq_channel_layer_process_script(struct SequenceChannelLayer *layer) {
    ...
    if (cmd != -1) {
        ...
        if ((layer->stopSomething == TRUE) && ((layer->note != NULL) || (layer->continuousNotes))) seq_channel_layer_note_decay(layer);
    }
}
...
#if defined(VERSION_EU) || defined(VERSION_SH)
                    if (!layer->ignoreDrumPan) layer->pan = drum->pan;
#else
...
#if defined(VERSION_EU) || defined(VERSION_SH)
                    instrument = ((layer->instOrWave == 0xffu) ? seqChannel->instrument : layer->instrument);
#else
                    ...
                    if (layer->portamento.mode != 0) {
                        ...
                        if (instrument != NULL) {
                            ...
                            sound = (((u8) vel <  instrument->normalRangeLo) ? &instrument->lowNotesSound
                                  : (((u8) vel <= instrument->normalRangeHi) ? &instrument->normalNotesSound : &instrument->highNotesSound));
#endif
                        ...
                        sound = ((cmd <  instrument->normalRangeLo) ? &instrument->lowNotesSound
                              : ((cmd <= instrument->normalRangeHi) ? &instrument->normalNotesSound : &instrument->highNotesSound));
#endif
                        sameSound        = (sound == layer->sound);
                        layer->sound     = sound;
...
s32 seq_channel_layer_process_script_part5(struct SequenceChannelLayer *layer, s32 cmd) {
    if (!layer->stopSomething && (layer->sound != NULL) && (layer->sound->sample->codec == CODEC_SKIP) && layer->sound->sample->medium != 0) {
        layer->stopSomething = TRUE;
        return -1;
    }
    if ((layer->continuousNotes == 1) && (layer->note != NULL) && layer->status && (cmd == 1) && (layer->note->parentLayer == layer)) {
...
s32 seq_channel_layer_process_script_part4(struct SequenceChannelLayer *layer, s32 cmd1) {
    ...
    if (temp == 0) { // drum
        ...
        if (cmd >= 0x80) {
            ...
            instrument = ((layer->instOrWave == 0xff) ? seqChannel->instrument : layer->instrument);
            if (layer->portamento.mode != 0) {
                vel = ((layer->portamentoTargetNote < cmd) ? cmd : layer->portamentoTargetNote);
                if (instrument != NULL) {
                    sound        = instrument_get_audio_bank_sound(instrument, vel);
...
// #endif
...
#ifdef VERSION_SH
                        cmd = m64_read_u8(state);
                        if (cmd & 0xf) seqChannel->notePriority = (cmd & 0xf);
                        cmd >>= 4;
                ...
                switch (cmd & 0xf0) {
                    ...
                    case 0x80: // chan_ioreadval; read data from audio lib
                        ...
                        if (loBits < 4) seqChannel->soundScriptIO[loBits] = -1;
                        break;
                    // sh: 0x50
                        ...
                        switch (seqPlayer->state) {
                            case SEQUENCE_PLAYER_STATE_2:
                                if (seqPlayer->fadeRemainingFrames != 0) {
                                    ...
                                    seqPlayer->fadeVelocity = ((targetVolume - seqPlayer->fadeVolume) / FLOAT_CAST(seqPlayer->fadeRemainingFrames));
                                    break;
                                }
                                // fallthrough
                            case SEQUENCE_PLAYER_STATE_0:
...
#else
                        if (IS_SEQUENCE_CHANNEL_VALID(seqPlayer->channels[loBits]) == TRUE) value = seqPlayer->channels[loBits]->finished;
#endif
                        break;
                    case 0x10:
                        break;
...
#if defined(VERSION_EU) || defined(VERSION_SH)
    for ((i = 0); (i < CHANNELS_MAX); (i++)) if (IS_SEQUENCE_CHANNEL_VALID(seqPlayer->channels[i]) == TRUE) sequence_channel_process_script(seqPlayer->channels[i]);
#else
    for ((i = 0); (i < CHANNELS_MAX); (i++)) if (seqPlayer->channels[i] != &gSequenceChannelNone) sequence_channel_process_script(seqPlayer->channels[i]);
#endif
}
