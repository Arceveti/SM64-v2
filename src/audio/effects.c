...
f32 get_vibrato_freq_scale(struct VibratoState *vib) {
    ...
    if (vib->extentChangeTimer) {
        ...
        if ((vib->extentChangeTimer = vib->seqChannel->vibratoExtentChangeDelay) == 0) vib->extent = (s32) vib->seqChannel->vibratoExtentTarget;
    }
    if (vib->rateChangeTimer) {
        ...
        if ((vib->rateChangeTimer = vib->seqChannel->vibratoRateChangeDelay) == 0) vib->rate = (s32) vib->seqChannel->vibratoRateTarget;
    }
