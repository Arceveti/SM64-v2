...
static inline void reverb_mono_sample(s16 *outSample, s32 inSample) {
    ...
    for (; (i != reverbFilterCount); ((++i), (++j))) {
        ...
        if (j == 2) {
            ...
            if (i != reverbFilterCountm1) tmpCarryover = ((tmpBufL[i] * REVERB_REV_INDEX) / 256);
        } else {
...
void prepare_reverb_ring_buffer(s32 chunkLen, u32 updateIndex) {
    ...
    if (gReverbDownsampleRate != 1) {
        ...
        if ((gSoundMode == SOUND_MODE_MONO) || monoReverb) {
            ...
            if (gReverbDownsampleRate != 1) {
                osInvalDCache(item->toDownsampleLeft, DEFAULT_LEN_2CH);
                for (((srcPos = 0), (dstPos = item->startPos)); (dstPos < ((item->lengthA / 2) + item->startPos)); ((srcPos += gReverbDownsampleRate), (dstPos++))) reverb_samples(&gSynthesisReverb.ringBuffer.left[dstPos], &gSynthesisReverb.ringBuffer.right[dstPos], item->toDownsampleLeft[srcPos], item->toDownsampleRight[srcPos]);
                for ((dstPos = 0); (dstPos < (item->lengthB / 2)); ((srcPos += gReverbDownsampleRate), (dstPos++))) reverb_samples(&gSynthesisReverb.ringBuffer.left[dstPos], &gSynthesisReverb.ringBuffer.right[dstPos], item->toDownsampleLeft[srcPos], item->toDownsampleRight[srcPos]);
            } else { // Too slow for practical use, not recommended most of the time.
                for ((dstPos = item->startPos); (dstPos < ((item->lengthA / 2) + item->startPos)); (dstPos++)) reverb_samples(&gSynthesisReverb.ringBuffer.left[dstPos], &gSynthesisReverb.ringBuffer.right[dstPos], gSynthesisReverb.ringBuffer.left[dstPos], gSynthesisReverb.ringBuffer.right[dstPos]);
                for ((dstPos = 0); (dstPos < (item->lengthB / 2)); (dstPos++)) reverb_samples(&gSynthesisReverb.ringBuffer.left[dstPos], &gSynthesisReverb.ringBuffer.right[dstPos], gSynthesisReverb.ringBuffer.left[dstPos], gSynthesisReverb.ringBuffer.right[dstPos]);
            }
        }
    }
#endif
...
#pragma GCC diagnostic pop
        ...
        for ((j = 0); (j < gNumSynthesisReverbs); (j++)) if (gSynthesisReverbs[j].useReverb != 0) prepare_reverb_ring_buffer(chunkLen, (gAudioBufferParameters.updatesPerFrame - i), j);
        cmd       = synthesis_do_one_audio_update((s16 *) aiBufPtr, chunkLen, cmd, (gAudioBufferParameters.updatesPerFrame - i));
        bufLen   -= chunkLen;
        aiBufPtr += chunkLen;
    }
    for ((j = 0); (j < gNumSynthesisReverbs); (j++)) {
        if (gSynthesisReverbs[j].framesLeftToIgnore != 0) gSynthesisReverbs[j].framesLeftToIgnore--;
...
u64 *synthesis_execute(u64 *cmdBuf, s32 *writtenCmds, s16 *aiBuf, s32 bufLen) {
    ...
    if (reverbFilterCount > NUM_ALLPASS) reverbFilterCount = NUM_ALLPASS;
    reverbFilterCountm1 = (reverbFilterCount - 1);
    if (reverbFilterCount < 3) reverbFilterCountm1 = 0;
#endif
    for ((i = gAudioUpdatesPerFrame); (i > 0); (i--)) {
        if (i == 1) {
            ...
            chunkLen = (v0 - (v0 & 0x7));
            if ((v0 & 7) >= 4) chunkLen += 8;
        }
        process_sequences(i - 1);
        if (gSynthesisReverb.useReverb != 0) prepare_reverb_ring_buffer(chunkLen, (gAudioUpdatesPerFrame - i));
        cmd       = synthesis_do_one_audio_update((s16 *) aiBufPtr, chunkLen, cmd, (gAudioUpdatesPerFrame - i));
        bufLen   -= chunkLen;
        aiBufPtr += chunkLen;
    }
    if (gSynthesisReverb.framesLeftToIgnore != 0) gSynthesisReverb.framesLeftToIgnore--;
...
u64 *synthesis_resample_and_mix_reverb(u64 *cmd, s32 bufLen, s16 reverbIndex, s16 updateIndex) {
    ...
    if (gSynthesisReverbs[reverbIndex].downsampleRate == 1) {
        ...
        if (item->lengthB != 0) cmd = synthesis_load_reverb_ring_buffer(cmd, (DMEM_ADDR_WET_LEFT_CH + item->lengthA), 0, item->lengthB, reverbIndex);
        ...
        if (item->lengthB != 0) cmd = synthesis_load_reverb_ring_buffer(cmd, (DMEM_ADDR_RESAMPLED + paddedLengthA), 0, (DEFAULT_LEN_1CH - paddedLengthA), reverbIndex);
    ...
    if (gSynthesisReverbs[reverbIndex].useReverb != 0) {
        switch (gSynthesisReverbs[reverbIndex].downsampleRate) {
            case 1:
                ...
                if (item->lengthB != 0) cmd = synthesis_save_reverb_ring_buffer(cmd, (DMEM_ADDR_WET_LEFT_CH + item->lengthA), 0, item->lengthB, reverbIndex); // Ring buffer wrapped
                break;
            default:
...
u64 *synthesis_do_one_audio_update(s16 *aiBuf, s32 bufLen, u64 *cmd, s32 updateIndex) {
    ...
    if (gNumSynthesisReverbs == 0) {
        for ((i = 0); (i < gMaxSimultaneousNotes); (i++)) if (gNoteSubsEu[gMaxSimultaneousNotes * updateIndex + i].enabled) noteIndices[notePos++] = i;
    } else {
        for ((j = 0); (j < gNumSynthesisReverbs); (j++)) {
            for ((i = 0); (i < gMaxSimultaneousNotes); (i++)) {
                noteSubEu = &gNoteSubsEu[(gMaxSimultaneousNotes * updateIndex) + i];
                if (noteSubEu->enabled && (j == noteSubEu->reverbIndex)) noteIndices[notePos++] = i;
            }
        }
        for ((i = 0); (i < gMaxSimultaneousNotes); (i++)) {
            noteSubEu = &gNoteSubsEu[(gMaxSimultaneousNotes * updateIndex) + i];
            if (noteSubEu->enabled && (noteSubEu->reverbIndex >= gNumSynthesisReverbs)) noteIndices[notePos++] = i;
        }
    }
    ...
    for ((j = 0); (j < gNumSynthesisReverbs); (j++)) {
        ...
        if (gUseReverb != 0) cmd = synthesis_resample_and_mix_reverb(cmd, bufLen, j, updateIndex);
        for (; (i < notePos); (i++)) {
        ...
        if (gSynthesisReverbs[j].useReverb != 0) cmd = synthesis_save_reverb_samples(cmd, j, updateIndex);
    }
    for (; (i < notePos); (i++)) {
        temp = updateIndex * gMaxSimultaneousNotes;
    ...
    for (noteIndex = 0; noteIndex < gMaxSimultaneousNotes; noteIndex++) {
        ...
        if (!note->noteSubEu.enabled) {
            ...
            if (note->frequency < US_FLOAT(2.0)) {
                nParts = 1;
                if (note->frequency > US_FLOAT(1.99996)) note->frequency = US_FLOAT(1.99996);
                resamplingRate = note->frequency;
            } else {
                // If frequency is > 2.0, the processing must be split into two parts
                nParts = 2;
                if (note->frequency >= US_FLOAT(3.99993)) note->frequency = US_FLOAT(3.99993);
...
#ifdef VERSION_EU
                    if (noteSubEu->bookOffset) curLoadedBook = euUnknownData_80301950; // what's this? never read
#endif
                        ...
                        case 2:
                            switch (curPart) {
                                case 0:
                                    ...
                                    break;
                                case 1:
                                    aSetBuffer(cmd++, 0, (DMEM_ADDR_UNCOMPRESSED_NOTE + sp130), DMEM_ADDR_RESAMPLED2, (samplesLenAdjusted + 8));
#ifdef VERSION_EU
                                    aResample(cmd++, A_INIT, 0xff60, VIRTUAL_TO_PHYSICAL2(synthesisState->synthesisBuffers->dummyResampleState));
#else
                                    aResample(cmd++, A_INIT, 0xff60, VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->dummyResampleState));
#endif
                                    aDMEMMove(cmd++, (DMEM_ADDR_RESAMPLED2 + 4), (DMEM_ADDR_RESAMPLED + resampledTempLen), (samplesLenAdjusted + 4));
                                    break;
                            }
                    }
...
#endif
#ifdef VERSION_EU
            if (noteSubEu->usesHeadsetPanEffects) cmd = note_apply_headset_pan_effects(cmd, noteSubEu, synthesisState, (bufLen * 2), flags, leftRight);
#else
            if (note->usesHeadsetPanEffects) cmd = note_apply_headset_pan_effects(cmd, note, (bufLen * 2), flags, leftRight);
#endif
        }
#ifndef VERSION_EU
    }
...
u64 *load_wave_samples(u64 *cmd, struct NoteSubEu *noteSubEu, struct NoteSynthesisState *synthesisState, s32 nSamplesToLoad) {
    ...
    if (a3 < nSamplesToLoad) {
        ...
        for ((i = 0); (i < repeats); (i++)) {
            aDMEMMove(cmd++, /*dmemin*/   DMEM_ADDR_UNCOMPRESSED_NOTE, /*dmemout*/ (DMEM_ADDR_UNCOMPRESSED_NOTE + ((1 + i) * 128)), /*count*/ 128);
        }
    }
    return cmd;
}
...
u64 *load_wave_samples(u64 *cmd, struct Note *note, s32 nSamplesToLoad) {
    s32 a3;
    s32 i;
    aSetBuffer (cmd++, /*flags*/ 0, /*dmemin*/ DMEM_ADDR_UNCOMPRESSED_NOTE, /*dmemout*/ 0, /*count*/ sizeof(note->synthesisBuffers->samples));
    ...
    if (a3 < nSamplesToLoad) for ((i = 0); (i <= (((nSamplesToLoad - a3 + 63) / 64) - 1)); i++) aDMEMMove(cmd++, /*dmemin*/ DMEM_ADDR_UNCOMPRESSED_NOTE, /*dmemout*/ (DMEM_ADDR_UNCOMPRESSED_NOTE + ((1 + i) * sizeof(note->synthesisBuffers->samples))), /*count*/ sizeof(note->synthesisBuffers->samples));
    return cmd;
}
#endif
...
u64 *process_envelope(u64 *cmd, struct NoteSubEu *note, struct NoteSynthesisState *synthesisState, s32 nSamples, u16 inBuf, s32 headsetPanSettings, UNUSED u32 flags) {
    s32 mixerFlags;
    s32 rampLeft, rampRight;
    u16 sourceLeft  = synthesisState->curVolLeft;
    u16 sourceRight = synthesisState->curVolRight;
    u16 targetLeft  = (note->targetVolLeft  << 5);
    u16 targetRight = (note->targetVolRight << 5);
    if (targetLeft  == 0) targetLeft++;
    if (targetRight == 0) targetRight++;
...
u64 *note_apply_headset_pan_effects(u64 *cmd, struct Note *note, s32 bufLen, s32 flags, s32 leftRight) {
    ...
    if (flags != 1) { // A_INIT?
        ...
        if (prevPanShift == 0) {
            ...
            aDMEMMove   (cmd++, DMEM_ADDR_NOTE_PAN_TEMP, (DMEM_ADDR_TEMP + 0x10), 0x10); // No idea, result seems to be overwritten later
...
void note_set_vel_pan_reverb(struct Note *note, f32 velocity, f32 pan, u8 reverbVol) {
    ...
    if (note->stereoHeadsetEffects && (gSoundMode == SOUND_MODE_HEADSET)) {
        ...
        s8 smallPanIndex            = ((temp < 9) ? temp : 9);
        ...
        Bool8 strongLeft            = FALSE;
        Bool8 strongRight           = FALSE;
...
#endif
    if (note->targetVolLeft  == 0) note->targetVolLeft++;
    if (note->targetVolRight == 0) note->targetVolRight++;
