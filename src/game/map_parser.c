...
static s32 headless_dma(u32 devAddr, void *dramAddr, u32 size) {
    ...
    while (stat & (PI_STATUS_IO_BUSY | PI_STATUS_DMA_BUSY)) stat = IO_READ(PI_STATUS_REG);
...
char *parse_map(u32 pc) {
    u32 i;
    for ((i = 0); (i < gMapEntrySize); (i++)) if (gMapEntries[i].addr >= pc) break;
...
char *find_function_in_stack(u32 *sp) {
    int i;
    for ((i = 0); (i < STACK_TRAVERSAL_LIMIT); (i++)) {
        u32 val = *sp;
        val     = *(u32 *)val;
        *sp    += 4;
