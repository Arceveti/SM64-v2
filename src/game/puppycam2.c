...
static uchar  gPCOptionStringsDE[][64] = {{NC_ANALOGUE_DE}, {NC_CAMX_DE}, {NC_CAMY_DE}, {NC_INVERTX_DE}, {NC_INVERTY_DE}, {NC_CAMC_DE}, {NC_SCHEME_DE}, {NC_WIDE_DE}, {OPTION_LANGUAGE_DE}, {NC_FOV_DE}, {NC_AA_DE}, {NC_SILHOUETTE_DE}};
static uchar  gPCFlagStringsFR  [][64] = {{OPTION_DISABLED_FR}, {OPTION_ENABLED_FR}, {OPTION_SCHEME1_FR}, {OPTION_SCHEME2_FR}, {OPTION_SCHEME3_FR}, {TEXT_ENGLISH}, {TEXT_FRENCH}, {TEXT_GERMAN},};
static uchar  gPCFlagStringsDE  [][64] = {{OPTION_DISABLED_DE}, {OPTION_ENABLED_DE}, {OPTION_SCHEME1_DE}, {OPTION_SCHEME2_DE}, {OPTION_SCHEME3_DE}, {TEXT_ENGLISH}, {TEXT_FRENCH}, {TEXT_GERMAN},};
static uchar  gPCToggleStringsFR[][64] = {{NC_BUTTON_FR}, {NC_BUTTON2_FR}, {NC_OPTION_FR}, {NC_HIGHLIGHT_L}, {NC_HIGHLIGHT_R},};
static uchar  gPCToggleStringsDE[][64] = {{NC_BUTTON_DE}, {NC_BUTTON2_DE}, {NC_OPTION_DE}, {NC_HIGHLIGHT_L}, {NC_HIGHLIGHT_R},};
#endif
static uchar  gPCOptionStringsEN[][64] = {{NC_ANALOGUE_EN}, {NC_CAMX_EN}, {NC_CAMY_EN}, {NC_INVERTX_EN}, {NC_INVERTY_EN}, {NC_CAMC_EN}, {NC_SCHEME_EN}, {NC_WIDE_EN}, {OPTION_LANGUAGE_EN}, {NC_FOV_EN}, {NC_AA_EN}, {NC_SILHOUETTE_EN}};
static uchar  gPCFlagStringsEN  [][64] = {{OPTION_DISABLED_EN}, {OPTION_ENABLED_EN}, {OPTION_SCHEME1_EN}, {OPTION_SCHEME2_EN}, {OPTION_SCHEME3_EN}, {TEXT_ENGLISH}, {TEXT_FRENCH}, {TEXT_GERMAN},};
...
#ifdef TARGET_N64
    if (!gPuppyCam.options.analogue) return;
...
void puppycam_boot(void) {
    vec3_set(gPuppyCam.zoomPoints, 600, 1000, 1500);
...
Bool32 puppycam_move_spline(struct sPuppySpline splinePos[], struct sPuppySpline splineFocus[], s32 mode, s32 index) {
    ...
    // vec3_copy(prevPos, gPuppyCam.pos);
    for ((i = 0); (i < 4); (i++)) vec3_copy(tempPoints[i], splinePos[gPuppyCam.splineIndex + i].pos);
    ...
    if (mode == PUPPYSPLINE_FOLLOW) {
        for ((i = 0); (i < 4); (i++)) vec3_copy(tempPoints[i], splineFocus[gPuppyCam.splineIndex + i].pos);
...
void puppycam_print_text(s32 x, s32 y, uchar *str, s32 col) {
    ...
    for ((i = 0); (i < gPCOptionCap); (i++)) {
        ...
        if ((scroll <= 140) && (scroll > 32)) {
            ...
            if (gPCOptions[i].gPCOptionStart != 255) {
                u32 var = (*gPCOptions[i].gPCOptionVar + gPCOptions[i].gPCOptionStart);
...
void puppycam_check_pause_buttons() {
    ...
    if (gPCOptionOpen) {
        if ((ABSI(gPlayer1Controller->rawStickY) > 60) || (gPlayer1Controller->buttonDown & (U_JPAD | D_JPAD))) {
            gPCOptionTimer--;
            if (gPCOptionTimer <= 0) {
                if (gPCOptionIndex == 0) {
                    gPCOptionIndex++;
        ...
        } else if ((ABSI(gPlayer1Controller->rawStickX) > 60) || (gPlayer1Controller->buttonDown & (L_JPAD | R_JPAD))) {
            gPCOptionTimer--;
            if (gPCOptionTimer <= 0) {
                if (gPCOptionIndex == 0) {
                    gPCOptionIndex++;
...
void puppycam_init(void) {
    ...
    vec3_copy(gPuppyCam.shake,        gVec3sZero);
    vec3_copy(gPuppyCam.pos,          gVec3sZero);
    vec3_copy(gPuppyCam.focus,        gVec3sZero);
    vec3_copy(gPuppyCam.pan,          gVec3sZero); // gMarioState->pos[1];
...
void puppycam_input_zoom(void) {
    ...
    if ((gPlayer1Controller->buttonPressed & R_TRIG) && (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_ZOOM_CHANGE)) {
        gPuppyCam.zoomSet++;
        if (gPuppyCam.zoomSet >= 3) gPuppyCam.zoomSet = 0;
...
static void puppycam_input_hold_preset3(void) {
    ...
    if (gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_IN) {
        ...
        if ((gPlayer1Controller->buttonPressed & L_TRIG) && (gPuppyCam.yawTarget % DEG(45))) gPuppyCam.yawTarget += (DEG(45) - (gPuppyCam.yawTarget % DEG(45)));

        if (gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_MED) approach_angle_bool(&gPuppyCam.pitchTarget, DEG(78.75), 0x200);
        if (gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_OUT) approach_angle_bool(&gPuppyCam.pitchTarget, DEG(67.5 ), 0x200);
...
static void puppycam_input_press(void) {
    ...
    f32 ivY = (((gPuppyCam.options.invertY * 2) - 1) * (gPuppyCam.options.sensitivityY / 100.f));
    // Analogue Camera stuff. If it fails to find an input, then it just sets stickmag to 100, which after calculations means the value goes unchanged.
    s8 stickMag = ((gPuppyCam.options.analogue) ? (gPuppyCam.stick2[0] * 1.25f) : 100);
    // Just in case it happens to be nonzero.
    gPuppyCam.yawAcceleration = 0;
    // In theory this shouldn't be necessary, but it's nice to cover all bases.
    if (!(gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_YAW_ROTATION)) return;
    if (((gPlayer1Controller->buttonPressed & L_CBUTTONS) && !gPuppyCam.options.analogue) || ((gPuppyCam.stickN[0] == 0) && (gPuppyCam.stick2[0] < -PUPPYCAM_DEADZONE))) {
        ...
        if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_INPUT_8DIR) {
            gPuppyCam.yawTarget -= (DEG(45) * ivX);
        } else {
            gPuppyCam.yawTarget -= (DEG(90) * ivX);
        }
        play_sound(SOUND_MENU_CAMERA_ZOOM_IN,gGlobalSoundSource);
...
void puppycam_debug_view(void) {
    if (gPuppyCam.debugFlags & PUPPYDEBUG_LOCK_CONTROLS) {
        f32 sinYaw = sins(gPuppyCam.yawTarget);
        f32 cosYaw = coss(gPuppyCam.yawTarget);
        if (ABSI(gPlayer1Controller->rawStickX) > PUPPYCAM_DEADZONE) {
            gPuppyCam.pos[0] += ((gPlayer1Controller->rawStickX / 4) * -sinYaw);
            gPuppyCam.pos[2] += ((gPlayer1Controller->rawStickX / 4) *  cosYaw);
        }
        if (ABSI(gPlayer1Controller->rawStickY) > PUPPYCAM_DEADZONE) {
            gPuppyCam.pos[0] += ((gPlayer1Controller->rawStickY / 4) *  cosYaw);
            gPuppyCam.pos[1] += ((gPlayer1Controller->rawStickY / 4) *  sins(gPuppyCam.pitchTarget));
            gPuppyCam.pos[2] += ((gPlayer1Controller->rawStickY / 4) *  sinYaw);
        }
        ...
        gPuppyCam.focus[0] = (gPuppyCam.pos[0] + (100 * cosYaw));
        gPuppyCam.focus[1] = (gPuppyCam.pos[1] + (100 * sins(gPuppyCam.pitchTarget)));
        gPuppyCam.focus[2] = (gPuppyCam.pos[2] + (100 * sinYaw));
    } else {
        if (gPuppyCam.debugFlags & PUPPYDEBUG_TRACK_MARIO) vec3_copy(gPuppyCam.focus, &gPuppyCam.targetObj->oPosVec);
    ...
    if ((gPlayer1Controller->buttonPressed & A_BUTTON) && (gPuppyCam.debugFlags & PUPPYDEBUG_LOCK_CONTROLS)) vec3_set(gMarioState->pos, gPuppyCam.pos[0], gPuppyCam.pos[1], gPuppyCam.pos[2]);
    ...
    if ((gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_PANSHIFT) && (gMarioState->action != ACT_HOLDING_BOWSER) && (gMarioState->action != ACT_SLEEPING) && (gMarioState->action != ACT_START_SLEEPING)) {
        ...
        if (gMarioState->vel[1] == 0.0f) {
            ...
            if (panFloor <= gPuppyCam.targetObj->oPosY - 150) {
                ...
                approach_angle_bool(&gPuppyCam.edgePitch, -DEG(45),  0x80);
            } else {
                approach_angle_bool(&gPuppyCam.edgePitch,      0x0, 0x100);
            }
...
void puppycam_terrain_angle(void) {
    ...
    if ((gMarioState->action & ACT_FLAG_SWIMMING) || !(gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_HEIGHT_HELPER))  {
        ...
        if (!farFromSurface && gotTheOkay) gPuppyCam.intendedTerrainPitch = approach_f32_asymptotic(gPuppyCam.intendedTerrainPitch, floorPitch, adjustSpeed * 3);
    }
    if (farFromSurface) gPuppyCam.terrainPitch = approach_f32_asymptotic(gPuppyCam.terrainPitch, gPuppyCam.intendedTerrainPitch, adjustSpeed);
}
...
void puppycam_wall_angle(void) {
    ...
    Angle wallYaw;
    if (!(gMarioState->action & ACT_WALL_KICK_AIR) || ((gMarioState->action & ACT_FLAG_AIR) && ABSF(gMarioState->forwardVel) < 16.0f) || !(gMarioState->action & ACT_FLAG_AIR)) return;
...
void puppycam_projection_behaviours(void) {
    ...
    if (gPuppyCam.targetObj == gMarioState->marioObj) {
        if ((gPuppyCam.options.turnAggression > 0)
         ...
         && (gPuppyCam.options.inputType != 2)) { // Holy hell this is getting spicy.
            ...
            if ((gPuppyCam.options.turnAggression > 0) || (gMarioState->action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE)) {
                ...
                if (((ABSI(gPlayer1Controller->rawStickX) > 20) && !(gMarioState->action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE)) ||
                    ...
                    approach_angle_bool(&gPuppyCam.yawTarget, (gMarioState->faceAngle[1] + DEG(180)), ((gPuppyCam.options.turnAggression * 10) * absf(gMarioState->forwardVel / 32) * absf(gPlayer1Controller->rawStickX / 80.0f) * turnRate));
                }
            }
            ...
            if ((gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_INPUT_8DIR) && (gPuppyCam.yawTarget % DEG(45))) gPuppyCam.yawTarget += (DEG(45) - (gPuppyCam.yawTarget % DEG(45)));
        ...
        if (!(gMarioState->action & ACT_FLAG_SWIMMING)) {
            ...
            approach_angle_bool(&gPuppyCam.yawTarget, (gMarioState->faceAngle[1] + DEG(180)), (1000 * (gMarioState->forwardVel / 32)));
...
UNUSED void puppycam_shake(UNUSED s16 x, UNUSED s16 y, UNUSED s16 z) {
    // Incomplete?
}
...
static void puppycam_projection(void) {
    ...
    if (gPuppyCam.targetObj) {
        vec3_set(targetPos, gPuppyCam.targetObj->oPosX, gPuppyCam.targetObj->oPosY, gPuppyCam.targetObj->oPosZ);
        vec3_copy(targetPos3, targetPos);
        if (gPuppyCam.targetObj2) {
            vec3_copy(targetPos2, &gPuppyCam.targetObj2->oPosVec);
            ...
            f32 dx = (targetPos[0] - targetPos2[0]);
            f32 dz = (targetPos[2] - targetPos2[2]);
            gPuppyCam.targetDist[0] = approach_f32_asymptotic(gPuppyCam.targetDist[0], (abss(LENCOS(sqrtf(sqr(dx) + sqr(dz)), abss((((gPuppyCam.yaw + DEG(180)) % 0xFFFF) - 0x8000) - (atan2s(dz, dx) % 0xFFFF) - 0x8000) + 0x4000))), 0.2f);
        } else {
...
static void puppycam_apply(void) {
    vec3_copy(gLakituState.pos,       gPuppyCam.pos  );
    vec3_copy(gLakituState.goalPos,   gPuppyCam.pos  );
    vec3_copy(gLakituState.curPos,    gPuppyCam.pos  );
    vec3_copy(gCamera->pos,           gPuppyCam.pos  );
    vec3_copy(sOldPosition,           gPuppyCam.pos  );
    vec3_copy(gLakituState.focus,     gPuppyCam.focus);
    vec3_copy(gLakituState.goalFocus, gPuppyCam.focus);
    vec3_copy(gLakituState.curFocus,  gPuppyCam.focus);
    vec3_copy(gCamera->focus,         gPuppyCam.focus);
    vec3_copy(sOldFocus,              gPuppyCam.focus);
    ...
    approach_angle_bool(&gLakituState.roll, 0x0, 0x80);
