...
Gfx dl_debug_box_verts[] = {
	...
	gsSP2Triangles(24, 25, 26, 0x0, 27, 25, 24, 0x0),
	gsSP2Triangles(28, 27, 24, 0x0, 24, 26, 28, 0x0),
	gsSP2Triangles(28, 26, 29, 0x0, 29, 26, 25, 0x0),
	gsSP2Triangles(29, 25, 30, 0x0, 27, 30, 25, 0x0),
	gsSP2Triangles(31, 30, 27, 0x0, 28, 31, 27, 0x0),
	gsSP2Triangles(28, 29, 31, 0x0, 31, 29, 30, 0x0),
	gsSPEndDisplayList(),
};
...
static void append_debug_box(Vec3f center, Vec3f bounds, Angle yaw, s32 type) {
    if (hitboxView) {
        ...
        if (!(sBoxes[sNumBoxes].type & DEBUG_UCODE_REJ) && !(sBoxes[sNumBoxes].type & DEBUG_UCODE_DEFAULT)) sBoxes[sNumBoxes].type |= DEBUG_UCODE_DEFAULT;
        ++sNumBoxes;
    }
}
...
static void render_box(int index) {
    ...
    if (box->type & DEBUG_SHAPE_BOX     ) gSPDisplayList(gDisplayListHead++, dl_debug_box_verts     );
    if (box->type & DEBUG_SHAPE_CYLINDER) gSPDisplayList(gDisplayListHead++, dl_debug_cylinder_verts);
    gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);
}
...
void render_debug_boxes(s32 type) {
    ...
    if (type & DEBUG_BOX_CLEAR) sNumBoxes = 0;
    gSPDisplayList(gDisplayListHead++, dl_debug_box_end);
}
