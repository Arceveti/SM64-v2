...
Bool32 mario_push_off_steep_floor(struct MarioState *m, MarioAction action, u32 actionArg) {
    if (abs_angle_diff(m->floorAngle, m->faceAngle[1]) < DEG(90)) {
        ...
        m->forwardVel   = -16.0f;// * (1.0f-m->floor->normal.y);
        m->faceAngle[1] = (m->floorAngle + DEG(180));
    }
...
static MarioStep perform_ground_quarter_step(struct MarioState *m, Vec3f nextPos) {
    ...
    if (upperWall.numWalls == 0) {
        ...
        for ((i = 0); (i < upperWall.numWalls); (i++)) {
            ...
            // m->wall = upperWall.walls[i];
            if ((wallDYaw >= DEG(60)) && (wallDYaw <= DEG(120))) continue;
    ...
    if (upperWall != NULL) {
        ...
        if ((wallDYaw >= DEG(60)) && (wallDYaw <= DEG(120))) return GROUND_STEP_NONE;
        return GROUND_STEP_HIT_WALL_CONTINUE_QSTEPS;
#endif
    }
    return GROUND_STEP_NONE;
...
#else
Bool32 check_ledge_grab(struct MarioState *m, struct Surface *wall, Vec3f intendedPos, Vec3f nextPos) {
    ...
    m->faceAngle[0] = 0x0;
    m->faceAngle[1] = (atan2s(wall->normal.z, wall->normal.x) + DEG(180));
    return TRUE;
}
#endif
