 ...
 * Mario.
 */
Angle bhv_falling_pillar_calculate_angle_in_front_of_mario(void) {
...
void bhv_falling_pillar_hitbox_loop(void) {
    ...
    o->oPosY = ((coss(pitch)             * yOffset) + pos[1]);
    o->oPosZ = ((sins(pitch) * coss(yaw) * yOffset) + pos[2]);
