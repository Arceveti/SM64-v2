...
void bhv_ukiki_cage_star_loop(void) {
    switch (o->oAction) {
        case UKIKI_CAGE_STAR_ACT_IN_CAGE:
            ...
            if ((o->oTimer == 0) && (save_file_get_star_flags((gCurrSaveFileNum - 1), (gCurrCourseNum - 1)) & STAR_FLAG_ACT_2)) cur_obj_set_model(MODEL_TRANSPARENT_STAR);
...
void ukiki_cage_act_wait_for_ukiki(void) {
    if (o->oUkikiCageNextAction != UKIKI_CAGE_ACT_WAIT_FOR_UKIKI) o->oAction = UKIKI_CAGE_ACT_SPIN;
...
void ukiki_cage_act_spin(void) {
    if (o->oUkikiCageNextAction != UKIKI_CAGE_ACT_SPIN) o->oAction = UKIKI_CAGE_ACT_FALL;
...
void ukiki_cage_act_fall(void) {
    ...
    if (o->oMoveFlags & (OBJ_MOVE_LANDED | OBJ_MOVE_ENTERED_WATER)) o->oAction = UKIKI_CAGE_ACT_HIDE;
}
