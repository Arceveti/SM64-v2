...
void bub_act_swimming_away_from_mario(void) {
    f32 dy;
    if (o->oTimer < 20) {
        if (o->oInteractStatus & INT_STATUS_INTERACTED) spawn_object(o, MODEL_WHITE_PARTICLE_SMALL, bhvSmallParticleSnow);
    } else {
        o->oInteractStatus = INT_STATUS_NONE;
    }
    if (o->oTimer == 0) cur_obj_play_sound_2(SOUND_GENERAL_MOVING_WATER);
    if (o->oForwardVel == 0.0f) o->oForwardVel = 6.0f;
    dy = o->oPosY - gMarioObject->oPosY;
    if (o->oPosY < o->oCheepCheepWaterLevel - 50.0f) {
        if (dy < 0.0f) dy = -dy;
...
void bhv_bub_loop(void) {
    ...
    if (o->parentObj->oAction == BUB_SPAWNER_ACT_REMOVE_BUBS) obj_mark_for_deletion(o);
}
