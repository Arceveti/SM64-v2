...
void bhv_animates_on_floor_switch_press_loop(void) {
    if (o->oFloorSwitchPressAnimationSwitchNotTicking != 0) {
        ...
        if (o->oFloorSwitchPressAnimationDoResetTime != 0) {
            o->oFloorSwitchPressAnimationTickTimer = D_80331ACC[o->oBehParams2ndByte];
        } else {
    ...
    o->collisionData = segmented_to_virtual(
        D_80331A54[o->oBehParams2ndByte][o->oFloorSwitchPressAnimationDoubleFrame / 2].unk00);

    cur_obj_set_model(D_80331A54[o->oBehParams2ndByte][o->oFloorSwitchPressAnimationDoubleFrame / 2].unk04);
}
