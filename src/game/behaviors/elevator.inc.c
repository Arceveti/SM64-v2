...
void elevator_act_4(void) {
    ...
    if (!mario_is_in_air_action() && !cur_obj_is_mario_on_platform()) o->oAction = 1;
}
...
void elevator_act_3(void) { // nearly identical to action 2
    ...
    if (!mario_is_in_air_action() && !cur_obj_is_mario_on_platform()) o->oAction = 0;
}

void bhv_elevator_init(void) {
    if (sElevatorHeights[o->oBehParams2ndByte * 3 + 2] == 0) {
