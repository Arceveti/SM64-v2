...
static void handle_merry_go_round_music(void) {
    ...
    if (!o->oMerryGoRoundMusicShouldPlay) {
        ...
        SurfaceType marioFloorType = ((gMarioState->floor == NULL) ? SURFACE_DEFAULT : gMarioState->floor->type);
...
void bhv_merry_go_round_loop(void) {
    ...
    if (!o->oMerryGoRoundMarioIsOutside) {
        ...
        if ((gMarioCurrentRoom != BBH_OUTSIDE_ROOM) && (gMarioCurrentRoom != BBH_DYNAMIC_SURFACE_ROOM)) o->oMerryGoRoundMarioIsOutside = FALSE;
    }
