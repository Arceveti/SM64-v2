 ...
 */
UNUSED void unused_init_free_list(struct LinkedList *usedList, struct LinkedList **pFreeList,
 ...
 */
UNUSED struct LinkedList *unused_try_allocate(struct LinkedList *destList, struct LinkedList *freeList) {
...
struct Object *create_object(const BehaviorScript *bhvScript) {
    ...
    //  themselves.
    switch (objListIndex) {
        case OBJ_LIST_GENACTOR: // fall through
        case OBJ_LIST_PUSHABLE: // fall through
        case OBJ_LIST_POLELIKE: snap_object_to_floor(obj); break;
        default:                                           break;
    }
    return obj;
}
