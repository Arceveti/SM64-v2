...
void *dynamic_dma_read(u8 *srcStart, u8 *srcEnd, u32 side, u32 alignment, u32 bssLength) {
    ...
    u32 size = ALIGN16(srcEnd - srcStart);
    u32 offset = 0;
    if (alignment && (side == MEMORY_POOL_LEFT)) offset = (ALIGN((uintptr_t)sPoolListHeadL + 16, alignment) - ((uintptr_t)sPoolListHeadL + 16));
    dest = main_pool_alloc((offset + size + bssLength), side);
    if (dest != NULL) {
        dma_read(((u8 *)dest + offset), srcStart, srcEnd);
        if (bssLength) bzero(((u8 *)dest + offset + size), bssLength);
    }
    return dest;
}
