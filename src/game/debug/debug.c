...
void print_mapinfo(void) {
    ...
    print_debug_top_down_mapinfo("wy   %d", o->oPosY);
    print_debug_top_down_mapinfo("wz   %d", o->oPosZ);
    ...
    if (o->oPosY < water) print_debug_top_down_mapinfo("water %d", water);
}
...
void try_do_mario_debug_object_spawn(void) {
    if (sDebugPage == DEBUG_PAGE_STAGEINFO && gDebugInfo[DEBUG_PAGE_ENEMYINFO][7] == 1) {
        if (gPlayer1Controller->buttonPressed & R_JPAD) spawn_object_relative(OBJ_BP_NONE, 0, 100, 200, o, MODEL_KOOPA_SHELL        , bhvKoopaShell          );
        if (gPlayer1Controller->buttonPressed & L_JPAD) spawn_object_relative(OBJ_BP_NONE, 0, 100, 200, o, MODEL_BREAKABLE_BOX_SMALL, bhvJumpingBox          );
        if (gPlayer1Controller->buttonPressed & D_JPAD) spawn_object_relative(OBJ_BP_NONE, 0, 100, 200, o, MODEL_KOOPA_SHELL        , bhvKoopaShellUnderwater);
        if (gPlayer1Controller->buttonPressed & U_JPAD) spawn_object_relative(OBJ_BP_NONE, 0, 100, 200, o, MODEL_BREAKABLE_BOX_SMALL, bhvBreakableBoxSmall   );
    }
}
