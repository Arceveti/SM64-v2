...
void play_knockback_sound(struct MarioState *m) {
    play_sound_if_no_flag(m, ((m->actionArg == 0) && ((m->forwardVel <= -28.0f) || (m->forwardVel >= 28.0f))) ? SOUND_MARIO_DOH : SOUND_MARIO_UH, MARIO_MARIO_SOUND_PLAYED);
}

MarioAction lava_boost_on_wall(struct MarioState *m) {
...
MarioAction check_fall_damage(struct MarioState *m, MarioAction hardFallAction) {
    f32 fallHeight;
    f32 damageHeight;
...
MarioAction check_kick_or_dive_in_air(struct MarioState *m) {
...
MarioAction should_get_stuck_in_ground(struct MarioState *m) {
...
MarioAction check_fall_damage_or_get_stuck(struct MarioState *m, MarioAction hardFallAction) {
...
MarioAction check_horizontal_wind(struct MarioState *m) {
...
u32 withTurn) {
    ...
    switch (stepResult) {
        ...
        case AIR_STEP_LANDED:
            if (!check_fall_damage_or_get_stuck(m, ACT_HARD_BACKWARD_GROUND_KB)) set_mario_action(m, landAction, 0);
            break;
...
MarioAction act_jump(struct MarioState *m) {
#ifdef ACTION_CANCELS
    if (m->actionTimer < 1) {
        m->actionTimer++;
        if ((m->input & INPUT_Z_PRESSED) && m->forwardVel > 10.0f) return set_mario_action(m, ACT_LONG_JUMP, 0);
    }
#endif
    ...
    common_air_action_step(m, ACT_JUMP_LAND, MARIO_ANIM_SINGLE_JUMP, AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG, TRUE);
    return FALSE;
}

MarioAction act_double_jump(struct MarioState *m) {
    ...
    common_air_action_step(m, ACT_DOUBLE_JUMP_LAND, animation, AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG, TRUE);
    return FALSE;
}

MarioAction act_triple_jump(struct MarioState *m) {
    if (gSpecialTripleJump        ) return set_mario_action(m, ACT_SPECIAL_TRIPLE_JUMP, 0);
...
#endif
    common_air_action_step(m, ACT_TRIPLE_JUMP_LAND, MARIO_ANIM_TRIPLE_JUMP, AIR_STEP_CHECK_HANG, TRUE);
#if ENABLE_RUMBLE
    if (m->action == ACT_TRIPLE_JUMP_LAND) queue_rumble_data(5, 40);
#endif
    play_flip_sounds(m, 2, 8, 20);
    return FALSE;
}

MarioAction act_backflip(struct MarioState *m) {
#ifdef ACTION_CANCELS
    ...
    common_air_action_step(m, ACT_BACKFLIP_LAND, MARIO_ANIM_BACKFLIP, 0, FALSE);
#if ENABLE_RUMBLE
    if (m->action == ACT_BACKFLIP_LAND) queue_rumble_data(5, 40);
#endif
    play_flip_sounds(m, 2, 3, 17);
    return FALSE;
}

MarioAction act_freefall(struct MarioState *m) {
    AnimID32 animation = MARIO_ANIM_SLOW_LEDGE_CLIMB; //!?
...
#endif
#ifdef ACTION_CANCELS
    if (!((m->prevAction & (ACT_FLAG_HANGING    | ACT_FLAG_ON_POLE    )) && (   m->actionTimer ==  0)) && check_kick_or_dive_in_air(m)) return TRUE;
    if ((!(m->prevAction & (ACT_FORWARD_ROLLOUT | ACT_BACKWARD_ROLLOUT)) || (++(m->actionTimer) > 10)) && (m->input & INPUT_Z_PRESSED)) return set_mario_action(m, ACT_GROUND_POUND, 0);
#else
    if (m->input & INPUT_B_PRESSED) return set_mario_action(m, ACT_DIVE        , 0);
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
#endif
    ...
    common_air_action_step(m, ACT_FREEFALL_LAND, animation, AIR_STEP_CHECK_LEDGE_GRAB, TRUE);
    return FALSE;
}

MarioAction act_hold_jump(struct MarioState *m) {
    if ((m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_OBJECT)                                ) return drop_and_set_mario_action(m, ACT_FREEFALL    , 0);
    if ((m->input & INPUT_B_PRESSED) && !(m->heldObj->oInteractionSubtype & INT_SUBTYPE_HOLDABLE_NPC)) return set_mario_action(         m, ACT_AIR_THROW   , 0);
    if ((m->input & INPUT_Z_PRESSED)                                                                 ) return drop_and_set_mario_action(m, ACT_GROUND_POUND, 0);
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
    common_air_action_step(m, ACT_HOLD_JUMP_LAND, MARIO_ANIM_JUMP_WITH_LIGHT_OBJ, AIR_STEP_CHECK_LEDGE_GRAB, TRUE);
    return FALSE;
}

MarioAction act_hold_freefall(struct MarioState *m) {
    AnimID32 animation = ((m->actionArg == 0) ? MARIO_ANIM_FALL_WITH_LIGHT_OBJ : MARIO_ANIM_FALL_FROM_SLIDING_WITH_LIGHT_OBJ);
    if ((m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_OBJECT)                                ) return drop_and_set_mario_action(m, ACT_FREEFALL    , 0);
    if ((m->input & INPUT_B_PRESSED) && !(m->heldObj->oInteractionSubtype & INT_SUBTYPE_HOLDABLE_NPC)) return set_mario_action(         m, ACT_AIR_THROW   , 0);
    if ((m->input & INPUT_Z_PRESSED)                                                                 ) return drop_and_set_mario_action(m, ACT_GROUND_POUND, 0);
    common_air_action_step(m, ACT_HOLD_FREEFALL_LAND, animation, AIR_STEP_CHECK_LEDGE_GRAB, TRUE);
    return FALSE;
}

MarioAction act_side_flip(struct MarioState *m) {
    if (m->input & INPUT_B_PRESSED) return set_mario_action(m, ACT_DIVE, 0);
    ...
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
    if (common_air_action_step(m, ACT_SIDE_FLIP_LAND, MARIO_ANIM_SLIDEFLIP, AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG, TRUE) != AIR_STEP_GRABBED_LEDGE) {
        m->marioObj->header.gfx.angle[1] += 0x8000;
    }
...
#ifdef WALL_SLIDE
MarioAction act_wall_slide(struct MarioState *m) {
...
MarioAction act_wall_kick_air(struct MarioState *m) {
#ifdef ACTION_CANCELS
    if (check_kick_or_dive_in_air(m)) return TRUE;
#else
    if (m->input & INPUT_B_PRESSED) return set_mario_action(m, ACT_DIVE        , 0);
    ...
    common_air_action_step(m, ACT_JUMP_LAND, MARIO_ANIM_SLIDEJUMP, AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG, (++(m->actionTimer) > 30));
    return FALSE;
}

MarioAction act_long_jump(struct MarioState *m) {
    AnimID32 animation;
#ifdef ACTION_CANCELS
    if (m->input & INPUT_B_PRESSED) return set_mario_action(m, ACT_DIVE, 0);
#endif
...
#if ENABLE_RUMBLE
    if (m->action == ACT_LONG_JUMP_LAND) queue_rumble_data(5, 40);
#endif
    return FALSE;
}

MarioAction act_riding_shell_air(struct MarioState *m) {
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
...
#endif
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:        set_mario_action(m, ACT_RIDING_SHELL_GROUND, 1); break;
...
MarioAction act_twirling(struct MarioState *m) {
    ...
    if (startTwirlYaw > m->twirlYaw) play_sound(SOUND_ACTION_TWIRL, m->marioObj->header.gfx.cameraToObject);
    update_lava_boost_or_twirling(m);
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:        set_mario_action(m, ACT_TWIRL_LAND, 0); break;
...
MarioAction act_dive(struct MarioState *m) {
    if (m->actionArg == 0) {
    ...
    if (mario_check_object_grab(m)) {
        ...
        if (m->action != ACT_DIVE) return TRUE;
    }
#ifdef AIR_TURN
    update_air_with_turn(m);
#else
...
MarioAction act_air_throw(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:
            if (!check_fall_damage_or_get_stuck(m, ACT_HARD_BACKWARD_GROUND_KB)) m->action = ACT_AIR_THROW_LAND;
            break;
        case AIR_STEP_HIT_WALL:      mario_set_forward_vel(m, 0.0f); break;
        case AIR_STEP_HIT_LAVA_WALL: lava_boost_on_wall(m);          break;
    }
...
MarioAction act_water_jump(struct MarioState *m) {
#ifdef ACTION_CANCELS
...
MarioAction act_hold_water_jump(struct MarioState *m) {
    if (m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_OBJECT) return drop_and_set_mario_action(m, ACT_FREEFALL, 0);
...
MarioAction act_steep_jump(struct MarioState *m) {
#ifdef ACTION_CANCELS
    if (check_kick_or_dive_in_air(m)) return TRUE;
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
#else
...
MarioAction act_ground_pound_jump(struct MarioState *m) {
    AnimID32 animation = (m->vel[1] >= 0.0f) ? MARIO_ANIM_DOUBLE_JUMP_RISE : MARIO_ANIM_DOUBLE_JUMP_FALL;
    if (check_kick_or_dive_in_air(m)) return TRUE;
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_HOOHOO);
...
MarioAction act_burning_jump(struct MarioState *m) {
...
MarioAction act_burning_fall(struct MarioState *m) {
    mario_set_forward_vel(m, m->forwardVel);
    if (perform_air_step(m, 0) == AIR_STEP_LANDED) {
        play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
        set_mario_action(m, ACT_BURNING_GROUND, 0);
...
MarioAction act_crazy_box_bounce(struct MarioState *m) {
    f32 minSpeed = 0.0f;
    if (m->actionTimer == 0) {
        switch (m->actionArg) {
        ...
        play_sound(((minSpeed < 40.0f) ? SOUND_GENERAL_CRAZY_BOX_BOING_SLOW : SOUND_GENERAL_CRAZY_BOX_BOING_FAST), m->marioObj->header.gfx.cameraToObject);
        if (m->forwardVel < minSpeed) mario_set_forward_vel(m, minSpeed);
        m->actionTimer = 1;
    }
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
    set_mario_animation(m, MARIO_ANIM_DIVE);
...
MarioStep common_air_knockback_step(struct MarioState *m, MarioAction landAction, MarioAction hardFallAction, AnimID32 animation, f32 speed) {
    ...
    switch (stepResult) {
        ...
        case AIR_STEP_LANDED:
#if ENABLE_RUMBLE
            if (m->action != ACT_SOFT_BONK) queue_rumble_data(5, 40);
#endif
            if (!check_fall_damage_or_get_stuck(m, hardFallAction)) set_mario_action(m, landAction, (m->action == ACT_THROWN_FORWARD || m->action == ACT_THROWN_BACKWARD) ? m->hurtCounter : m->actionArg);
            break;
        case AIR_STEP_HIT_WALL:
...
MarioAction check_wall_kick(struct MarioState *m) {
...
MarioAction act_backward_air_kb(struct MarioState *m) {
    if (check_wall_kick(m)) return TRUE;
#ifdef ACTION_CANCELS
    if (check_kick_or_dive_in_air(m)) return TRUE;
...
MarioAction act_forward_air_kb(struct MarioState *m) {
    if (check_wall_kick(m)) return TRUE;
#ifdef ACTION_CANCELS
    if (check_kick_or_dive_in_air(m)) return TRUE;
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
...
MarioAction act_thrown_forward(struct MarioState *m) {
    Angle pitch;
...
MarioAction act_soft_bonk(struct MarioState *m) {
    if (check_wall_kick(m)) return TRUE;
#ifdef ACTION_CANCELS
    if (m->actionTimer++ > 1) return set_mario_action(m, ACT_FREEFALL, 0);
    if (m->actionArg == 0) play_knockback_sound(m);
#else
    play_knockback_sound(m);
#endif
...
MarioAction act_getting_blown(struct MarioState *m) { // :flushed:
    if (m->actionState == 0) {
...
MarioAction act_air_hit_wall(struct MarioState *m) {
#ifdef WALL_SLIDE
    Angle wallAngle;
    ...
    if (m->wall != NULL) {
        ...
        if (++(m->actionTimer) <= 2) {
            ...
            if ((m->prevAction != ACT_WALL_SLIDE)
             && (m->intendedMag > 16.0f)
             && abs_angle_diff(atan2s(m->wall->normal.z, m->wall->normal.x), m->intendedYaw) <= 0x2000) {
...
MarioAction act_forward_rollout(struct MarioState *m) {
    if (m->actionState == 0) {
        m->vel[1]      = 30.0f;
        m->actionState = 1;
    }
...
MarioAction act_backward_rollout(struct MarioState *m) {
    if (m->actionState == 0) {
        m->vel[1]      = 30.0f;
        m->actionState = 1;
    }
...
MarioAction act_butt_slide_air(struct MarioState *m) {
...
MarioAction act_hold_butt_slide_air(struct MarioState *m) {
...
MarioAction act_lava_boost(struct MarioState *m) {
#if COYOTE_TIME > 0
    m->coyoteTimer = 0;
#endif
#if ENABLE_RUMBLE
...
MarioAction act_slide_kick(struct MarioState *m) {
    if (m->actionState == 0 && m->actionTimer == 0) {
        play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_HOOHOO);
        set_mario_animation(m, MARIO_ANIM_SLIDE_KICK);
    }
    if ((++(m->actionTimer) > 30) && ((m->pos[1] - m->floorHeight) > 500.0f)) return set_mario_action(m, ACT_FREEFALL, 2);
#ifdef AIR_TURN
    update_air_with_turn(m);
#else
    update_air_without_turn(m);
...
MarioAction act_jump_kick(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:
            if (!check_fall_damage_or_get_stuck(m, ACT_HARD_BACKWARD_GROUND_KB)) set_mario_action(m, ACT_FREEFALL_LAND, 0);
            break;
        case AIR_STEP_HIT_WALL: mario_set_forward_vel(m, 0.0f); break;
    }
    return FALSE;
...
MarioAction act_shot_from_cannon(struct MarioState *m) {
...
MarioAction act_flying(struct MarioState *m) {
    Angle startPitch = m->faceAngle[0];
#ifdef REONU_CAM_3
    if (gPlayer1Controller->buttonPressed & R_TRIG) gFlyingCamOverride ^= TRUE;
#endif
    if (m->input & INPUT_Z_PRESSED) {
        if (m->area->camera->mode == CAMERA_MODE_BEHIND_MARIO) set_camera_mode(m->area->camera, m->area->camera->defMode, 1);
    ...
    switch (perform_air_step(m, 0)) {
        ...
        case AIR_STEP_HIT_WALL:
            if (m->wall != NULL) {
                ...
                if (m->actionTimer++ == 0) play_sound(SOUND_ACTION_HIT, m->marioObj->header.gfx.cameraToObject);
                if (m->actionTimer == 30)  m->actionTimer = 0;
                m->faceAngle[0] -= 0x200;
                if (m->faceAngle[0] < -0x2AAA) m->faceAngle[0] = -0x2AAA;
                m->marioObj->header.gfx.angle[0] = -m->faceAngle[0];
                m->marioObj->header.gfx.angle[2] =  m->faceAngle[2];
...
MarioAction act_riding_hoot(struct MarioState *m) {
...
MarioAction act_flying_triple_jump(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:
            if (!check_fall_damage_or_get_stuck(m, ACT_HARD_BACKWARD_GROUND_KB)) set_mario_action(m, ACT_DOUBLE_JUMP_LAND, 0);
            break;
        case AIR_STEP_HIT_WALL:      mario_bonk_reflection(m, FALSE); break;
        case AIR_STEP_HIT_LAVA_WALL: lava_boost_on_wall(m);           break;
    }
...
MarioAction act_top_of_pole_jump(struct MarioState *m) {
#ifdef ACTION_CANCELS
    if (m->input & INPUT_B_PRESSED) return check_kick_or_dive_in_air(m);
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
#endif
    play_mario_jump_sound(m);
...
MarioAction act_vertical_wind(struct MarioState *m) {
...
MarioAction act_special_triple_jump(struct MarioState *m) {
    if (m->input & INPUT_B_PRESSED) return set_mario_action(m, ACT_DIVE        , 0);
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_YAHOO);
#ifdef AIR_TURN
        ...
        case AIR_STEP_HIT_WALL:
            ...
            if (m->forwardVel > 16.0f) {
                ...
                if (m->wall != NULL) {
                    ...
                    if (m->vel[1] > 0.0f) m->vel[1] = 0.0f;
                    if (m->forwardVel >= 38.0f) {
                        m->particleFlags |= PARTICLE_19; //? PARTICLE_VERTICAL_STAR
                        set_mario_action(m, ACT_BACKWARD_AIR_KB, 0);
                    } else {
                        if (m->forwardVel > 8.0f) mario_set_forward_vel(m, -8.0f);
                        return set_mario_action(m, ACT_SOFT_BONK, 0);
...
MarioAction check_common_airborne_cancels(struct MarioState *m) {
...
MarioAction mario_execute_airborne_action(struct MarioState *m) {
    ...
    if (check_common_airborne_cancels(m)) return TRUE;
    ...
    switch (m->action) {
        ...
        case ACT_RIDING_SHELL_JUMP:    // fall through
        ...
        default:                       cancel = act_freefall(m);             break;
    }
    /* clang-format on */
    return cancel;
}
