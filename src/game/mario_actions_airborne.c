...
#ifdef AIR_TURN
        m->slideVelX = ((m->forwardVel * sins(m->faceAngle[1])) + (sidewaysSpeed * sins(m->faceAngle[1] + DEGREES(90))));
        m->slideVelZ = ((m->forwardVel * coss(m->faceAngle[1])) + (sidewaysSpeed * coss(m->faceAngle[1] + DEGREES(90))));
#ifdef GRAVITY_DRAG
        if (m->vel[1] < 0.0f) {
...
void update_flying(struct MarioState *m) {
    ...
    if (m->faceAngle[0] >  DEGREES(60)) m->faceAngle[0] =  DEGREES(60);
    if (m->faceAngle[0] < -DEGREES(60)) m->faceAngle[0] = -DEGREES(60);
...
Bool32 act_jump(struct MarioState *m) {
#ifdef ACTION_CANCELS
    if (m->actionTimer < 1) {
        m->actionTimer++;
...
Bool32 act_triple_jump(struct MarioState *m) {
...
Bool32 act_backflip(struct MarioState *m) {
#ifdef ACTION_CANCELS
...
Bool32 act_freefall(struct MarioState *m) {
...
Bool32 act_hold_jump(struct MarioState *m) {
...
Bool32 act_hold_freefall(struct MarioState *m) {
...
Bool32 act_wall_slide(struct MarioState *m) {
    ...
    if (m->input & INPUT_NONZERO_ANALOG) {
        ...
        if ((m->intendedMag > 16.0f) && (wallDintendedYaw <= DEGREES(45))) {
            m->faceAngle[1] = m->intendedYaw;
...
Bool32 act_wall_kick_air(struct MarioState *m) {
#ifdef ACTION_CANCELS
    if (check_kick_or_dive_in_air(m)) return TRUE;
#else
    if (m->input & INPUT_B_PRESSED) return set_mario_action(m, ACT_DIVE        , 0);
...
Bool32 act_riding_shell_air(struct MarioState *m) {
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, 0);
...
Bool32 act_air_throw(struct MarioState *m) {
...
Bool32 act_water_jump(struct MarioState *m) {
#ifdef ACTION_CANCELS
...
Bool32 act_hold_water_jump(struct MarioState *m) {
    if (m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_OBJECT) return drop_and_set_mario_action(m, ACT_FREEFALL, 0);
...
Bool32 act_steep_jump(struct MarioState *m) {
#ifdef ACTION_CANCELS
    if (check_kick_or_dive_in_air(m)) return TRUE;
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
#else
...
Bool32 act_ground_pound(struct MarioState *m) {
    MarioStep stepResult;
    f32 yOffset;
...
Bool32 act_ground_pound_jump(struct MarioState *m) {
    AnimID32 animation = (m->vel[1] >= 0.0f) ? MARIO_ANIM_DOUBLE_JUMP_RISE : MARIO_ANIM_DOUBLE_JUMP_FALL;
    if (check_kick_or_dive_in_air(m)) return TRUE;
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_HOOHOO);
...
Bool32 act_burning_jump(struct MarioState *m) {
...
Bool32 act_burning_fall(struct MarioState *m) {
    mario_set_forward_vel(m, m->forwardVel);
    if (perform_air_step(m, 0) == AIR_STEP_LANDED) {
        play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
        set_mario_action(m, ACT_BURNING_GROUND, 0);
...
Bool32 act_crazy_box_bounce(struct MarioState *m) {
    f32 minSpeed = 0.0f;
    if (m->actionTimer == 0) {
        switch (m->actionArg) {
...
Bool32 check_wall_kick(struct MarioState *m) {
...
Bool32 act_backward_air_kb(struct MarioState *m) {
...
Bool32 act_forward_air_kb(struct MarioState *m) {
    if (check_wall_kick(m)) return TRUE;
#ifdef ACTION_CANCELS
    if (check_kick_or_dive_in_air(m)) return TRUE;
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
...
Bool32 act_soft_bonk(struct MarioState *m) {
    if (check_wall_kick(m)) return TRUE;
#ifdef ACTION_CANCELS
...
AnimFrame32 act_air_hit_wall(struct MarioState *m) {
    ...
    if (m->wall != NULL) {
        ...
        m->faceAngle[1] = (((2.0f * wallAngle) - m->faceAngle[1]) + DEGREES(180));
        if (++(m->actionTimer) <= 2) {
            ...
            if ((m->prevAction != ACT_WALL_SLIDE)
             && (m->intendedMag > 16.0f)
             && abs_angle_diff(atan2s(m->wall->normal.z, m->wall->normal.x), m->intendedYaw) <= DEGREES(45)) {
                m->faceAngle[1] = (m->intendedYaw + DEGREES(180));
                m->forwardVel   = 0.0f;
                return set_mario_action(m, ACT_FREEFALL, 0);
            }
...
Bool32 act_lava_boost(struct MarioState *m) {
#if COYOTE_TIME > 0
    m->coyoteTimer = 0;
#endif
#if ENABLE_RUMBLE
...
Bool32 act_shot_from_cannon(struct MarioState *m) {
...
Bool32 act_flying(struct MarioState *m) {
...
Bool32 act_riding_hoot(struct MarioState *m) {
    ...
    vec3f_copy(m->marioObj->header.gfx.pos, m->pos);
    vec3a_set(m->marioObj->header.gfx.angle, 0x0, (DEGREES(90) - m->faceAngle[1]), 0x0);
    return FALSE;
}

Bool32 act_flying_triple_jump(struct MarioState *m) {
...
Bool32 act_top_of_pole_jump(struct MarioState *m) {
#ifdef ACTION_CANCELS
...
Bool32 act_vertical_wind(struct MarioState *m) {
...
Bool32 act_special_triple_jump(struct MarioState *m) {
    if (m->input & INPUT_B_PRESSED) return set_mario_action(m, ACT_DIVE        , 0);
    if (m->input & INPUT_Z_PRESSED) return set_mario_action(m, ACT_GROUND_POUND, 0);
    play_mario_sound(m, SOUND_ACTION_TERRAIN_JUMP, SOUND_MARIO_YAHOO);
#ifdef AIR_TURN
