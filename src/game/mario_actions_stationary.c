...
Bool32 check_common_idle_cancels(struct MarioState *m) {
    ...
    if (m->input & INPUT_Z_DOWN      ) return set_mario_action(          m, ACT_START_CROUCHING , 0);
    return FALSE;
}
...
Bool32 check_common_hold_idle_cancels(struct MarioState *m) {
    ...
    if (m->input & INPUT_Z_DOWN        ) return drop_and_set_mario_action(m, ACT_START_CROUCHING   , 0);
    return FALSE;
}
...
Bool32 act_idle(struct MarioState *m) {
    ...
    if (m->actionArg & 0x1) {
        ...
        switch (m->actionState) {
            case ACT_IDLE_STATE_HEAD_LEFT:   set_mario_animation(m, MARIO_ANIM_IDLE_HEAD_LEFT  ); break;
            case ACT_IDLE_STATE_HEAD_RIGHT:  set_mario_animation(m, MARIO_ANIM_IDLE_HEAD_RIGHT ); break;
            case ACT_IDLE_STATE_HEAD_CENTER: set_mario_animation(m, MARIO_ANIM_IDLE_HEAD_CENTER); break;
        }
        if (is_anim_at_end(m)) {
            ...
            if (++m->actionState == ACT_IDLE_STATE_SLEEP) {
                ...
                if ((deltaYOfFloorBehindMario < -24.0f) || (24.0f < deltaYOfFloorBehindMario) || (m->floor->flags & SURFACE_FLAG_DYNAMIC)) {
                    ...
                    m->actionTimer++;
                    if (m->actionTimer < 10) m->actionState = 0;
                }
            }
        }
    }
...
Bool32 act_start_sleeping(struct MarioState *m) {
    ...
    if (check_common_idle_cancels(m)) return TRUE;
    if (m->quicksandDepth > 30.0f) return set_mario_action(m, ACT_IN_QUICKSAND, 0);
    if (m->actionState == ACT_START_SLEEPING_STATE_SLEEP) return set_mario_action(m, ACT_SLEEPING, 0);
    switch (m->actionState) {
        case ACT_START_SLEEPING_STATE_IDLE:     animFrame = set_mario_animation(m, MARIO_ANIM_START_SLEEP_IDLE   );                                                       break;
        case ACT_START_SLEEPING_STATE_SCRATCH:  animFrame = set_mario_animation(m, MARIO_ANIM_START_SLEEP_SCRATCH);                                                       break;
        case ACT_START_SLEEPING_STATE_YAWN:     animFrame = set_mario_animation(m, MARIO_ANIM_START_SLEEP_YAWN   ); m->marioBodyState->eyeState = MARIO_EYES_HALF_CLOSED; break;
        case ACT_START_SLEEPING_STATE_SITTING:  animFrame = set_mario_animation(m, MARIO_ANIM_START_SLEEP_SITTING); m->marioBodyState->eyeState = MARIO_EYES_HALF_CLOSED; break;
        default:                                animFrame = set_mario_animation(m, MARIO_ANIM_START_SLEEP_IDLE   );                                                       break;
    }
    ...
    if (animFrame == -1) {
        if (m->actionState == ACT_START_SLEEPING_STATE_YAWN   ) play_sound(SOUND_MARIO_YAWNING  , m->marioObj->header.gfx.cameraToObject);
        if (m->actionState == ACT_START_SLEEPING_STATE_SCRATCH) play_sound(SOUND_MARIO_IMA_TIRED, m->marioObj->header.gfx.cameraToObject);
    }
    stationary_ground_step(m);
    return FALSE;
}
...
Bool32 act_sleeping(struct MarioState *m) {
    ...
    if (m->input
        & (INPUT_NONZERO_ANALOG
         ...
         | INPUT_STOMPED
         | INPUT_B_PRESSED
         | INPUT_Z_PRESSED)      ) return set_mario_action(m, ACT_WAKING_UP, m->actionState);
    if (m->quicksandDepth > 30.0f) return set_mario_action(m, ACT_WAKING_UP, m->actionState);
    ...
    switch (m->actionState) {
        case ACT_SLEEPING_STATE_IDLE:
            ...
            if ( animFrame ==  2) play_sound(SOUND_MARIO_SNORING1, m->marioObj->header.gfx.cameraToObject);
            if ( animFrame == 20) play_sound(SOUND_MARIO_SNORING2, m->marioObj->header.gfx.cameraToObject);
            if (is_anim_at_end(m)) {
                m->actionTimer++;
                if (m->actionTimer > 45) m->actionState = ACT_SLEEPING_STATE_START_LYING;
            }
            break;
        case ACT_SLEEPING_STATE_START_LYING:
            if (set_mario_animation(m, MARIO_ANIM_SLEEP_START_LYING) == 18) play_mario_heavy_landing_sound(m, SOUND_ACTION_TERRAIN_BODY_HIT_GROUND);
            if (is_anim_at_end(m)) m->actionState = ACT_SLEEPING_STATE_LYING;
            break;
...
#else
            if (animFrame ==  2) play_sound(SOUND_MARIO_SNORING2, m->marioObj->header.gfx.cameraToObject);
            if (animFrame == 25) play_sound(SOUND_MARIO_SNORING1, m->marioObj->header.gfx.cameraToObject);
#endif
            break;
    }
    return FALSE;
...
Bool32 act_waking_up(struct MarioState *m) {
    ...
    if (m->input & INPUT_ABOVE_SLIDE) return set_mario_action(m, ACT_BEGIN_SLIDING   , 0);
    m->actionTimer++;
    if (m->actionTimer > 20) return set_mario_action(m, ACT_IDLE, 0);
    stationary_ground_step(m);
...
Bool32 act_shivering(struct MarioState *m) {
    ...
    stationary_ground_step(m);
    switch (m->actionState) {
        case ACT_SHIVERING_STATE_WARMING_HAND:
            ...
            if ((animFrame == 7) || (animFrame == 81)) play_sound(SOUND_ACTION_CLAP_HANDS_COLD, m->marioObj->header.gfx.cameraToObject);
            if (is_anim_past_end(m)) m->actionState = ACT_SHIVERING_STATE_SHAKE;
            break;
        case ACT_SHIVERING_STATE_SHAKE:
            animFrame = set_mario_animation(m, MARIO_ANIM_SHIVERING);
            if ((animFrame == 9 )
             || (animFrame == 25)
             || (animFrame == 44)) play_sound(SOUND_ACTION_CLAP_HANDS_COLD, m->marioObj->header.gfx.cameraToObject);
            break;
        case ACT_SHIVERING_STATE_RETURN_TO_IDLE:
            set_mario_animation(m, MARIO_ANIM_SHIVERING_RETURN_TO_IDLE);
            if (is_anim_past_end(m)) set_mario_action(m, ACT_IDLE, 0);
            break;
    }
    return FALSE;
}
...
Bool32 act_hold_idle(struct MarioState *m) {
...
Bool32 act_hold_heavy_idle(struct MarioState *m) {
    if (m->input & INPUT_STOMPED       ) return drop_and_set_mario_action(m, ACT_SHOCKWAVE_BOUNCE  , 0);
    ...
    if (m->input & INPUT_NONZERO_ANALOG) return set_mario_action(         m, ACT_HOLD_HEAVY_WALKING, 0);
...
Bool32 act_in_quicksand(struct MarioState *m) {
    if (m->quicksandDepth < 30.0f) return set_mario_action(m, ACT_IDLE, 0);
    if (check_common_idle_cancels(m)) return TRUE;
    set_mario_animation(m, ((m->quicksandDepth > 70.0f) ? MARIO_ANIM_DYING_IN_QUICKSAND : MARIO_ANIM_IDLE_IN_QUICKSAND));
    stationary_ground_step(m);
    return FALSE;
}
...
Bool32 act_crouching(struct MarioState *m) {
    ...
    if (m->input & INPUT_OFF_FLOOR     ) return set_mario_action(  m, ACT_FREEFALL        , 0);
    if (m->input & INPUT_ABOVE_SLIDE   ) return set_mario_action(  m, ACT_BEGIN_SLIDING   , 0);
    if (m->input & INPUT_FIRST_PERSON  ) return set_mario_action(  m, ACT_STOP_CROUCHING  , 0);
    if (!(m->input & INPUT_Z_DOWN)     ) return set_mario_action(  m, ACT_STOP_CROUCHING  , 0);
    if (m->input & INPUT_NONZERO_ANALOG) return set_mario_action(  m, ACT_START_CRAWLING  , 0);
    if (m->input & INPUT_B_PRESSED     ) return set_mario_action(  m, ACT_PUNCHING        , 9);
    stationary_ground_step(m);
...
Bool32 act_panting(struct MarioState *m) {
    if (m->input & INPUT_STOMPED    ) return set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    if (m->health >= 0x500          ) return set_mario_action(m, ACT_IDLE            , 0);
    if (check_common_idle_cancels(m)) return TRUE;
    if (set_mario_animation(m, MARIO_ANIM_WALK_PANTING) == 1) play_sound((SOUND_MARIO_PANTING + ((gAudioRandom % 3U) << 0x10)), m->marioObj->header.gfx.cameraToObject);
    stationary_ground_step(m);
    m->marioBodyState->eyeState = MARIO_EYES_HALF_CLOSED;
    return FALSE;
}

Bool32 act_hold_panting_unused(struct MarioState *m) {
    if (m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_OBJECT) return drop_and_set_mario_action(m, ACT_PANTING         , 0);
    if (m->input & INPUT_STOMPED                                   ) return drop_and_set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    if (m->health >= 0x500                                         ) return set_mario_action(         m, ACT_HOLD_IDLE       , 0);
    if (check_common_hold_idle_cancels(m)) return TRUE;
...
Bool32 act_butt_slide_stop(struct MarioState *m) {
    ...
    if (m->marioObj->header.gfx.animInfo.animFrame == 6) play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
    return FALSE;
}
...
Bool32 act_hold_butt_slide_stop(struct MarioState *m) {
    if (m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_OBJECT) return drop_and_set_mario_action(m, ACT_IDLE            , 0);
    if (m->input & INPUT_STOMPED                                   ) return drop_and_set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    if (m->input & (INPUT_NONZERO_ANALOG | INPUT_A_PRESSED | INPUT_OFF_FLOOR | INPUT_ABOVE_SLIDE)) return check_common_hold_action_exits(m);
    if (m->input & INPUT_B_PRESSED                                 ) return set_mario_action(         m, ACT_THROWING        , 0);
...
#endif
    if (is_anim_past_end(m)) set_mario_action(m, ACT_CROUCHING, 0);
    return FALSE;
}
...
#endif
    if (is_anim_past_end(m)) set_mario_action(m, ACT_CRAWLING, 0);
    return FALSE;
}
...
Bool32 check_common_landing_cancels(struct MarioState *m, MarioAction action) {
    if (m->input & INPUT_STOMPED     ) return set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    if (m->input & INPUT_FIRST_PERSON) return set_mario_action(m, ACT_IDLE            , 0);
...
Bool32 act_backflip_land_stop(struct MarioState *m) {
    if (!(m->input & INPUT_Z_DOWN) || m->marioObj->header.gfx.animInfo.animFrame >= 6) m->input &= ~INPUT_A_PRESSED;
    if (check_common_landing_cancels(m, ACT_BACKFLIP)) return TRUE;
    landing_step(m, MARIO_ANIM_TRIPLE_JUMP_LAND, ACT_IDLE);
    return FALSE;
}
...
Bool32 act_twirl_land(struct MarioState *m) { //! actionState name?
    m->actionState = 1;
    if (m->input & INPUT_STOMPED  ) return set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    if (m->input & INPUT_OFF_FLOOR) return set_mario_action(m, ACT_FREEFALL        , 0);
    stationary_ground_step(m);
...
Bool32 act_ground_pound_land(struct MarioState *m) { //! actionState name?
    m->actionState = 1;
    if (m->input & INPUT_STOMPED    ) return drop_and_set_mario_action(m, ACT_SHOCKWAVE_BOUNCE , 0);
    if (m->input & INPUT_OFF_FLOOR  ) return set_mario_action(         m, ACT_FREEFALL         , 0);
...
Bool32 act_first_person(struct MarioState *m) {
    if (m->actionState == ACT_FIRST_PERSON_STATE_SET_MODE) {
        lower_background_noise(2);
        set_camera_mode(m->area->camera, CAMERA_MODE_C_UP, 0x10);
        m->actionState = ACT_FIRST_PERSON_STATE_IDLE;
...
Bool32 check_common_stationary_cancels(struct MarioState *m) {
    ...
    if ((m->action != ACT_NO_STANDING_DEATH) && (m->health < 0x100)) {
        update_mario_sound_and_camera(m);
...
Bool32 mario_execute_stationary_action(struct MarioState *m) {
    Bool32 cancel;
    if (check_common_stationary_cancels(m)) return TRUE;
    ...
    switch (m->action) {
        ...
        default:                          cancel = act_idle(m);                             break;
    }
    /* clang-format on */
