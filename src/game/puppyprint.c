...
void puppyprint_profiler_process(void) {
    ...
    if (fDebug) {
        if (gPlayer1Controller->buttonPressed & D_JPAD) {
            benchViewer ^= 1;
            ramViewer = 0;
        } else if (gPlayer1Controller->buttonPressed & U_JPAD && !(gPlayer1Controller->buttonPressed & L_TRIG)) {
            ramViewer ^= 1;
            benchViewer = 0;
        }
        if (benchViewer) {
...
void print_small_text(s32 x, s32 y, const char *str, s32 align, s32 amount) {
    ...
    for (i = 0; i < tx; i++) {
        ...
        if (str[i] == '<') i += text_iterate_command(str, i, TRUE);
        if (shakeToggle) {
