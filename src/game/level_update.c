...
#define WARP_NODE_CREDITS_MIN   WARP_NODE_CREDITS_START
...
struct CreditsEntry sCreditsSequence[] = {
    ...
    { LEVEL_JRB,            1,   18,   22, {  3800, -4840,  2727 }, CREDITS_POS_TWO,   credits03 },
    { LEVEL_CCM,            2,   34,   25, { -5464,  6656, -6575 }, CREDITS_POS_THREE, credits04 },
    { LEVEL_BBH,            1,    1,   60, {   256,  1922,  2580 }, CREDITS_POS_ONE,   credits05 },
    { LEVEL_HMC,            1,  -15,  123, { -6469,  1616, -6054 }, CREDITS_POS_FOUR,  credits06 },
    { LEVEL_THI,            3,   17,  -32, {   508,  1024,  1942 }, CREDITS_POS_TWO,   credits07 },
    { LEVEL_LLL,            2,   33,  124, {   -73,    82, -1467 }, CREDITS_POS_THREE, credits08 },
    { LEVEL_SSL,            1,   65,   98, { -5906,  1024, -2576 }, CREDITS_POS_ONE,   credits09 },
    { LEVEL_DDD,            1,   50,   47, { -4884, -4607,  -272 }, CREDITS_POS_FOUR,  credits10 },
    { LEVEL_SL,             1,   17,  -34, {  1925,  3328,   563 }, CREDITS_POS_TWO,   credits11 },
    { LEVEL_WDW,            1,   33,  105, {  -537,  1850,  1818 }, CREDITS_POS_THREE, credits12 },
    { LEVEL_TTM,            1,    2,  -33, {  2613,   313,  1074 }, CREDITS_POS_ONE,   credits13 },
    { LEVEL_THI,            1,   51,   54, { -2609,   512,   856 }, CREDITS_POS_FOUR,  credits14 },
    { LEVEL_TTC,            1,   17,  -72, { -1304,   -71,  -967 }, CREDITS_POS_TWO,   credits15 },
    { LEVEL_RR,             1,   33,   64, {  1565,  1024,  -148 }, CREDITS_POS_THREE, credits16 },
    { LEVEL_SA,             1,    1,   24, { -1050, -1330, -1559 }, CREDITS_POS_ONE,   credits17 },
    { LEVEL_COTMC,          1,   49,  -16, {  -256,   415, -6045 }, CREDITS_POS_FOUR,  credits18 },
    { LEVEL_DDD,            2, -111,  -64, {  3948,  1185,  -104 }, CREDITS_POS_TWO,   credits19 },
    { LEVEL_CCM,            1,   33,   31, {  3169, -4607,  5240 }, CREDITS_POS_THREE, credits20 },
    // End Level Credits Sequence
    { LEVEL_CASTLE_GROUNDS, 1,    1, -128, {     0,   906, -1200 }, CREDITS_POS_ONE,   NULL      },
    { LEVEL_NONE,           0,    1,    0, {     0,     0,     0 }, CREDITS_POS_ONE,   NULL      },
};
...
void load_level_init_text(u32 arg) {
    ...
    switch (dialogID) {
        ...
        case (u8)DIALOG_NONE: gotAchievement = TRUE;                                                      break; // 255, cast value to u8 to match (-1)
        default: gotAchievement = save_file_get_star_flags((gCurrSaveFileNum - 1), (gCurrCourseNum - 1)); break;
    }
    if (!gotAchievement) {
...
void init_door_warp(struct SpawnInfo *spawnInfo, u32 warpDestFlags) {
    if (warpDestFlags & WARP_FLAG_DOOR_FLIP_MARIO) spawnInfo->startAngle[1] += DEG(180);
...
void set_mario_initial_cap_powerup(struct MarioState *m) {
    ...
    switch (capCourseIndex) {
        case (COURSE_COTMC - COURSE_CAP_COURSES): m->flags |= (MARIO_METAL_CAP  | MARIO_CAP_ON_HEAD); m->capTimer =  600; break;
        case (COURSE_TOTWC - COURSE_CAP_COURSES): m->flags |= (MARIO_WING_CAP   | MARIO_CAP_ON_HEAD); m->capTimer = 1200; break;
        case (COURSE_VCUTM - COURSE_CAP_COURSES): m->flags |= (MARIO_VANISH_CAP | MARIO_CAP_ON_HEAD); m->capTimer =  600; break;
    }
}
...
void init_mario_after_warp(void) {
    ...
    if (gMarioState->action != ACT_UNINITIALIZED) {
        ...
        if (marioSpawnType == MARIO_SPAWN_DOOR_WARP) init_door_warp(&gPlayerSpawnInfos[0], sWarpDest.flags);
        ...
        init_mario();
        set_mario_initial_action(gMarioState, marioSpawnType, sWarpDest.flags);
    ...
    if (gCurrDemoInput == NULL) {
        ...
        if (gMarioState->flags & (MARIO_VANISH_CAP | MARIO_WING_CAP)                                                                                                                                  ) play_cap_music(              SEQUENCE_ARGS(4, SEQ_EVENT_POWERUP  )   );
        if ((gCurrLevelNum == LEVEL_BOB) && (get_current_background_music() != SEQUENCE_ARGS(4, SEQ_LEVEL_SLIDE)) && sTimerRunning                                                                    ) play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(4, SEQ_LEVEL_SLIDE    ), 0);
        if ((sWarpDest.levelNum == LEVEL_CASTLE)         && (sWarpDest.areaIdx == 1) && ((sWarpDest.nodeId == 31) || (sWarpDest.nodeId == 32)                                                        )) play_sound(SOUND_MENU_MARIO_CASTLE_WARP, gGlobalSoundSource);
        if ((sWarpDest.levelNum == LEVEL_CASTLE_GROUNDS) && (sWarpDest.areaIdx == 1) && ((sWarpDest.nodeId ==  7) || (sWarpDest.nodeId == 10) || (sWarpDest.nodeId == 20) || (sWarpDest.nodeId == 30))) play_sound(SOUND_MENU_MARIO_CASTLE_WARP, gGlobalSoundSource);
    }
}
...
void warp_credits(void) {
    ...
    if ((gCurrCreditsEntry == NULL) || (gCurrCreditsEntry == sCreditsSequence)) set_background_music(gCurrentArea->musicParam, gCurrentArea->musicParam2, 0);
}
 ...
 */
void initiate_warp(s16 destLevel, s16 destArea, s16 destWarpNode, s32 warpDestFlags) {
    ...
    if (sDelayedWarpOp == WARP_OP_NONE) {
        ...
        switch (warpOp) {
            ...
            case WARP_OP_SPIN_SHRINK: // bbh enter
                sDelayedWarpTimer = 30;
                sSourceWarpNodeId = ((m->usedObj->oBehParams & 0x00FF0000) >> 16); //! param mask names
                play_transition(WARP_TRANSITION_FADE_INTO_COLOR, sDelayedWarpTimer, 0xFF, 0xFF, 0xFF);
                break;
        ...
        if (fadeMusic && (gCurrDemoInput == NULL)) fadeout_music(((3 * sDelayedWarpTimer / 2) * 8) - 2);
    }
        ...
        if (gDebugLevelSelect && (sDelayedWarpOp & WARP_OP_TRIGGERS_LEVEL_SELECT)) {
            ...
            switch (sDelayedWarpOp) {
                ...
                case WARP_OP_CREDITS_START:
                    ...
                    initiate_warp(gCurrCreditsEntry->levelNum, gCurrCreditsEntry->areaIndex, WARP_NODE_CREDITS_START, WARP_FLAGS_NONE);
                    break;
                case WARP_OP_CREDITS_NEXT:
...
void update_hud_values(void) {
    if (gCurrCreditsEntry == NULL) {
        ...
        if ((gHudDisplay.coins < gMarioState->numCoins) && (gGlobalTimer & 0x1)) {
 ...
 */
Bool32 play_mode_frame_advance(void) {
 ...
 */
Bool32 play_mode_change_area(void) {
    // sm64ex-axo
    ...
    // }
    // if (sTransitionTimer > 0) sTransitionTimer--;
 ...
 * Play the transition and then return to normal play mode.
 */
Bool32 play_mode_change_level(void) {
    if (sTransitionUpdate != NULL) sTransitionUpdate(&sTransitionTimer);
    if (--sTransitionTimer == -1) {
        gHudDisplay.flags   = HUD_DISPLAY_NONE;
        sTransitionTimer    = 0;
        sTransitionUpdate   = NULL;
        return ((sWarpDest.type != WARP_TYPE_NOT_WARPING) ? sWarpDest.levelNum : sSpecialWarpDest);
    }
    return FALSE;
}
 ...
 */
static Bool32 play_mode_unused(void) {
    if (--sTransitionTimer == -1) {
        gHudDisplay.flags = HUD_DISPLAY_NONE;
        return ((sWarpDest.type != WARP_TYPE_NOT_WARPING) ? sWarpDest.levelNum : sSpecialWarpDest);
    }
...
Bool32 update_level(void) {
    Bool32 changeLevel = FALSE;
...
Bool32 init_level(void) {
    ...
    if (sWarpDest.type != WARP_TYPE_NOT_WARPING) {
        ...
        if (gCurrDemoInput == NULL) set_background_music(gCurrentArea->musicParam, gCurrentArea->musicParam2, 0);
    }
#if ENABLE_RUMBLE
...
Bool32 lvl_set_current_level(UNUSED Bool32 initOrUpdate, s32 levelNum) {
    Bool32 warpCheckpointActive = sWarpCheckpointActive;
