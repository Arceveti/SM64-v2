...
void _Litob(printf_struct *args, u8 type) {
    ...
    num_map = (type == 'X') ? D_80334974 : D_80334960;

    base = (type == 'o') ? 8 : ((type != 'x' && type != 'X') ? 10 : 16);
    buff_ind = BUFF_LEN;
    num = args->value.s64;

    if ((type == 'd' || type == 'i') && args->value.s64 < 0) num = -num;

    if (num != 0 || args->precision != 0) buff[--buff_ind] = num_map[num % base];
    ...
    if (args->part2_len < args->precision) args->num_leading_zeros = args->precision - args->part2_len;
    ...
    if (args->precision < 0 && (args->flags & (FLAGS_ZERO | FLAGS_MINUS)) == FLAGS_ZERO) {
        ...
        if (buff_ind > 0) args->num_leading_zeros += buff_ind;
    }
}
