...
s32 act_reading_sign(struct MarioState *m) {
    ...
    switch (m->actionState) {
        ...
        // turn toward sign
        case 1:
            m->faceAngle[1] += marioObj->oMarioPoleUnused / 11;
...
s32 act_warp_door_spawn(struct MarioState *m) {
    if (m->actionState == 0) {
        m->actionState = 1;
        if (m->actionArg & 1) { // sDelayedWarpArg
            m->usedObj->oInteractStatus = INT_STATUS_WARP_DOOR_ANIM_3;
        } else {
            m->usedObj->oInteractStatus = INT_STATUS_WARP_DOOR_ANIM_4;
        }
