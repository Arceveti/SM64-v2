...
static s8 sPeachIsBlinking        = FALSE; // or isn't blinking?
...
void print_displaying_credits_entry(void) {
    ...
    if (sDispCreditsEntry != NULL) {
        ...
        titleStr  = *currStrPtr++;
        numLines  = (*titleStr++ - '0');
        strY      = ((sDispCreditsEntry->posVpAndText & CREDITS_POS_THREE) ? 28 : 172) + (numLines == 1) * 16;
#ifndef VERSION_JP
        lineHeight = 16;
#endif
...
void bhv_end_toad_loop(void) {
    s32 toadAnimIndex = (o->oPosX >= 0.0f);
    cur_obj_init_animation_with_sound(sEndToadAnims[toadAnimIndex]);
    // 0-1, 2-3, 4, 5, 6, 7
    if (cur_obj_check_if_near_animation_end() && ((sEndToadAnims[toadAnimIndex] == 0) || (sEndToadAnims[toadAnimIndex] == 2))) sEndToadAnims[toadAnimIndex]++;
}
...
Gfx *geo_switch_peach_eyes(s32 callContext, struct GraphNode *node, UNUSED s32 context) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        if (sPeachManualBlinkTime == 0) {
            ...
            switchCase->selectedCase = ((sPeachIsBlinking * 4) + ((timer < 7) ? sPeachBlinkTimes[timer] : 1));
        } else {
...
s32 mario_ready_to_speak(void) {
    MarioAction actionGroup = (gMarioState->action & ACT_GROUP_MASK);
    return (((gMarioState->action == ACT_WAITING_FOR_DIALOG) || (actionGroup == ACT_GROUP_STATIONARY) || (actionGroup == ACT_GROUP_MOVING))
       && (!(gMarioState->action & (ACT_FLAG_RIDING_SHELL | ACT_FLAG_INVULNERABLE)) && gMarioState->action != ACT_FIRST_PERSON));
}
...
// 23: end
Bool32 act_reading_npc_dialog(struct MarioState *m) { // actionState is used as a timer
    s32 headTurnAmount = 0;
    Angle angleToNPC   = 0x0;
    Angle turnSpeed    = DEG(11.25);
    ...
    if (m->actionState < 8) {
        ...
        angleToNPC = mario_obj_angle_to_object(m, m->usedObj);
#endif
        approach_s16_symmetric_bool(&m->faceAngle[1], angleToNPC, turnSpeed);
        // turn head to npc
...
Bool32 act_disappeared(struct MarioState *m) {
    set_mario_animation(m, MARIO_ANIM_A_POSE);
    stop_and_set_height_to_floor(m);
    m->marioObj->header.gfx.node.flags &= ~GRAPH_RENDER_ACTIVE;
    if (m->actionArg) {
        m->actionArg--;
        if (!(m->actionArg & 0xFFFF)) level_trigger_warp(m, (m->actionArg >> 16));
    }
    return FALSE;
}

Bool32 act_reading_automatic_dialog(struct MarioState *m) { // actionState is used as a timer
    u32 actionArg;
    m->actionState++;
...
Bool32 act_reading_sign(struct MarioState *m) {
    ...
    switch (m->actionState) {
        case ACT_READING_SIGN_STATE_START_DIALOG:
            start_object_cutscene_without_focus(CUTSCENE_READ_MESSAGE);
            enable_time_stop();
            // reading sign
            set_mario_animation(m, MARIO_ANIM_FIRST_PERSON);
            m->actionState = ACT_READING_SIGN_STATE_TURN_TOWARD_SIGN;
            // intentional fall through
        case ACT_READING_SIGN_STATE_TURN_TOWARD_SIGN:
            m->faceAngle[1] += (marioObj->oMarioReadingSignDYaw  / 0xB);
            ...
            if (m->actionTimer++ == 10) {
                ...
                m->actionState = ACT_READING_SIGN_STATE_IN_DIALOG;
            }
            break;
        case ACT_READING_SIGN_STATE_IN_DIALOG:
...
Bool32 act_debug_free_move(struct MarioState *m) {
    ...
    if (gPlayer1Controller->buttonDown & D_JPAD) pos[1] -= (16.0f * speed);
    if (m->intendedMag > 0) {
...
void general_star_dance_handler(struct MarioState *m, Bool32 isInWater) {
    DialogID dialogID;
    if (m->actionState == ACT_STAR_DANCE_STATE_CUTSCENE) {
        switch (++m->actionTimer) {
            case 1:
                ...
                if (m->actionArg & 0x1) {
                    ...
                    play_music(SEQ_PLAYER_ENV, SEQUENCE_ARGS(15, ((gCurrLevelNum == LEVEL_BOWSER_1) || (gCurrLevelNum == LEVEL_BOWSER_2)) ? SEQ_EVENT_CUTSCENE_COLLECT_KEY : SEQ_EVENT_CUTSCENE_COLLECT_STAR), 0);
                }
                break;
            case 42:
            ...
            case 80:
                if (!(m->actionArg & 0x1)) {
                    ...
                    m->actionState = ACT_STAR_DANCE_STATE_DO_SAVE;
                }
                break;
        }
    } else if ((m->actionState == ACT_STAR_DANCE_STATE_DO_SAVE) && (gDialogResponse != DIALOG_RESPONSE_NONE)) {
        if (gDialogResponse == DIALOG_RESPONSE_YES) save_file_do_save(gCurrSaveFileNum - 1);
        m->actionState = ACT_STAR_DANCE_STATE_RETURN;
    } else if ((m->actionState == ACT_STAR_DANCE_STATE_RETURN) && is_anim_at_end(m)) {
        disable_time_stop();
...
Bool32 act_star_dance(struct MarioState *m) {
    m->faceAngle[1] = m->area->camera->yaw;
    set_mario_animation(m, ((m->actionState == ACT_STAR_DANCE_STATE_RETURN) ? MARIO_ANIM_RETURN_FROM_STAR_DANCE : MARIO_ANIM_STAR_DANCE));
    general_star_dance_handler(m, FALSE);
    if ((m->actionState != 2) && (m->actionTimer >= 40)) m->marioBodyState->handState = MARIO_HAND_PEACE_SIGN;
    stop_and_set_height_to_floor(m);
    return FALSE;
}
...
Bool32 act_fall_after_star_grab(struct MarioState *m) {
...
Bool32 act_standing_death(struct MarioState *m) {
    if (m->input & INPUT_IN_POISON_GAS) return set_mario_action(m, ACT_SUFFOCATION, 0);
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    common_death_handler(m, MARIO_ANIM_DYING_FALL_OVER, 80);
    if (m->marioObj->header.gfx.animInfo.animFrame == 77) play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_BODY_HIT_GROUND);
    return FALSE;
}

Bool32 act_electrocution(struct MarioState *m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    common_death_handler(m, MARIO_ANIM_ELECTROCUTION, 43);
    return FALSE;
}

Bool32 act_suffocation(struct MarioState *m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    common_death_handler(m, MARIO_ANIM_SUFFOCATING, 86);
    return FALSE;
}

Bool32 act_death_on_back(struct MarioState *m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    if (common_death_handler(m, MARIO_ANIM_DYING_ON_BACK, 54) == 40) play_mario_heavy_landing_sound(m, SOUND_ACTION_TERRAIN_BODY_HIT_GROUND);
    return FALSE;
}

Bool32 act_death_on_stomach(struct MarioState *m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    if (common_death_handler(m, MARIO_ANIM_DYING_ON_STOMACH, 37) == 37) play_mario_heavy_landing_sound(m, SOUND_ACTION_TERRAIN_BODY_HIT_GROUND);
    return FALSE;
}

Bool32 act_quicksand_death(struct MarioState *m) {
    if (m->actionState == ACT_QUICKSAND_DEATH_INIT) {
        set_mario_animation(m, MARIO_ANIM_DYING_IN_QUICKSAND);
        set_anim_to_frame(m, 60);
        m->actionState = ACT_QUICKSAND_DEATH_SINK;
        m->actionTimer = 0;
    }
    if (m->actionState == ACT_QUICKSAND_DEATH_SINK) {
        ...
        if (m->actionArg == 0) {
            if ((m->quicksandDepth += 5.0f) >= 180.0f) {
                ...
                m->actionState = ACT_QUICKSAND_DEATH_DEAD;
            }
        } else { // Quicksand wall
    ...
    if (m->actionArg == 0) stationary_ground_step(m);
    play_sound(SOUND_MOVING_QUICKSAND_DEATH, m->marioObj->header.gfx.cameraToObject);
    return FALSE;
}

Bool32 act_eaten_by_bubba(struct MarioState *m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    set_mario_animation(m, MARIO_ANIM_A_POSE);
    m->marioObj->header.gfx.node.flags &= ~GRAPH_RENDER_ACTIVE;
#ifdef BREATH_METER
...
// set animation and forwardVel; when perform_air_step returns AIR_STEP_LANDED, set the new action
s32 launch_mario_until_land(struct MarioState *m, MarioAction endAction, AnimID32 animation, f32 forwardVel) {
    s32 airStepLanded;
    mario_set_forward_vel(m, forwardVel);
    set_mario_animation(m, animation);
    airStepLanded = (perform_air_step(m, 0) == AIR_STEP_LANDED);
    if (airStepLanded) set_mario_action(m, endAction, 0);
...
Bool32 act_unlocking_key_door(struct MarioState *m) {
    ...
    if (m->actionArg & WARP_FLAG_DOOR_FLIP_MARIO) m->faceAngle[1] += DEG(180);
    if (m->actionTimer == 0) {
    ...
    switch (m->marioObj->header.gfx.animInfo.animFrame) {
        case  79: play_sound(SOUND_GENERAL_DOOR_INSERT_KEY, m->marioObj->header.gfx.cameraToObject); break;
        case 111: play_sound(SOUND_GENERAL_DOOR_TURN_KEY,   m->marioObj->header.gfx.cameraToObject); break;
    }
...
Bool32 act_unlocking_star_door(struct MarioState *m) {
    switch (m->actionState) {
        case ACT_UNLOCKING_STAR_DOOR_STATE_MOVE_POS:
            m->faceAngle[1] = m->usedObj->oMoveAngleYaw;
            if (m->actionArg & WARP_FLAG_DOOR_FLIP_MARIO) m->faceAngle[1] += DEG(180);
            ...
            m->actionState = ACT_UNLOCKING_STAR_DOOR_STATE_SUMMON_STAR;
            break;
        case ACT_UNLOCKING_STAR_DOOR_STATE_SUMMON_STAR:
            if (is_anim_at_end(m)) {
                spawn_object(m->marioObj, MODEL_STAR, bhvUnlockDoorStar);
                m->actionState = ACT_UNLOCKING_STAR_DOOR_STATE_APPROACH_DOOR;
            }
            break;
        case ACT_UNLOCKING_STAR_DOOR_STATE_APPROACH_DOOR:
            if (m->actionTimer++ == 70) {
                set_mario_animation(m, MARIO_ANIM_RETURN_STAR_APPROACH_DOOR);
                m->actionState = ACT_UNLOCKING_STAR_DOOR_STATE_IN_DIALOG;
            }
            break;
        case ACT_UNLOCKING_STAR_DOOR_STATE_IN_DIALOG:
            if (is_anim_at_end(m)) {
...
Bool32 act_entering_star_door(struct MarioState *m) {
    ...
    stop_and_set_height_to_floor(m);
    if (m->actionTimer == 48) set_mario_action(m, ACT_IDLE, 0);
    return FALSE;
}

Bool32 act_going_through_door(struct MarioState *m) {
    if (m->actionTimer == 0) {
...
Bool32 act_warp_door_spawn(struct MarioState *m) {
    if (m->actionState == ACT_WARP_DOOR_SPAWN_SET_INTERACT_STATUS) {
        m->actionState = ACT_WARP_DOOR_EXIT_DOOR;
        m->usedObj->oInteractStatus = ((m->actionArg & WARP_FLAG_DOOR_PULLED) ? INT_STATUS_WARP_DOOR_PULLED : INT_STATUS_WARP_DOOR_PUSHED); // sDelayedWarpArg
...
Bool32 act_emerge_from_pipe(struct MarioState *m) {
    struct Object *marioObj = m->marioObj;
    if (m->actionTimer++ < 11) {
    ...
    if (gCurrLevelNum == LEVEL_THI) play_sound_if_no_flag(m, ((gCurrAreaIndex == 2) ? SOUND_MENU_EXIT_PIPE : SOUND_MENU_ENTER_PIPE), MARIO_ACTION_SOUND_PLAYED);
...
Bool32 act_spawn_spin_airborne(struct MarioState *m) {
    ...
    if ((m->actionState == ACT_SPAWN_SPIN_AIRBORNE_STATE_SPINNING) && ((m->pos[1] - m->floorHeight) > 300.0f)) {
        if (set_mario_animation(m, MARIO_ANIM_FORWARD_SPINNING) == 0) play_sound(SOUND_ACTION_SPIN, m->marioObj->header.gfx.cameraToObject); // first anim frame
    } else { // under 300 units above floor, enter freefall animation
        m->actionState = ACT_SPAWN_SPIN_AIRBORNE_STATE_FALLING;
        set_mario_animation(m, MARIO_ANIM_GENERAL_FALL);
    }
    return FALSE;
}

Bool32 act_spawn_spin_landing(struct MarioState *m) {
    stop_and_set_height_to_floor(m);
 ...
 */
Bool32 act_exit_airborne(struct MarioState *m) {
#ifdef BREATH_METER
    m->breath = 0x880;
#endif
...
Bool32 act_falling_exit_airborne(struct MarioState *m) {
#ifdef BREATH_METER
    m->breath = 0x880;
#endif
...
Bool32 act_exit_land_save_dialog(struct MarioState *m) {
    AnimFrame32 animFrame;
    stationary_ground_step(m);
    play_mario_landing_sound_once(m, SOUND_ACTION_TERRAIN_LANDING);
    switch (m->actionState) {
        case ACT_EXIT_LAND_SAVE_DIALOG_STATE_DETERMINE_TYPE: // determine type of exit
            ...
            if (is_anim_past_end(m)) {
                ...
                if (!(m->flags & MARIO_CAP_ON_HEAD)) m->actionState = ACT_EXIT_LAND_SAVE_DIALOG_STATE_NO_CAP; // star exit without cap
                if ((gLastCompletedCourseNum == COURSE_BITDW) || (gLastCompletedCourseNum == COURSE_BITFS)) m->actionState = ACT_EXIT_LAND_SAVE_DIALOG_STATE_KEY; // key exit
            }
            break;
        case ACT_EXIT_LAND_SAVE_DIALOG_STATE_KEY:
            ...
            handle_save_menu(m);
            break;
        case ACT_EXIT_LAND_SAVE_DIALOG_STATE_NO_CAP:
            ...
            handle_save_menu(m);
            break;
        case ACT_EXIT_LAND_SAVE_DIALOG_STATE_WITH_CAP:
            animFrame = set_mario_animation(m, MARIO_ANIM_TAKE_CAP_OFF_THEN_ON);
            switch (animFrame) {
                case 12: cutscene_take_cap_off(m);                                                    break;
                case 37: // fall through
                case 53: play_sound(SOUND_ACTION_BRUSH_HAIR, m->marioObj->header.gfx.cameraToObject); break;
                case 82: cutscene_put_cap_on(m);                                                      break;
            }
            handle_save_menu(m);
            break;
    }
...
Bool32 act_death_exit(struct MarioState *m) {
    if (15 < m->actionTimer++
...
Bool32 act_unused_death_exit(struct MarioState *m) {
...
Bool32 act_falling_death_exit(struct MarioState *m) {
...
Bool32 act_special_exit_airborne(struct MarioState *m) {
    struct Object *marioObj = m->marioObj;
    play_sound_if_no_flag(m, SOUND_MARIO_YAHOO, MARIO_MARIO_SOUND_PLAYED);
    if (m->actionTimer++ < 11) {
        marioObj->header.gfx.node.flags &= ~GRAPH_RENDER_ACTIVE;
...
Bool32 act_special_death_exit(struct MarioState *m) {
    struct Object *marioObj = m->marioObj;
    if (m->actionTimer++ < 11) {
        marioObj->header.gfx.node.flags &= ~GRAPH_RENDER_ACTIVE;
        return FALSE;
...
Bool32 act_spawn_no_spin_airborne(struct MarioState *m) {
...
Bool32 act_spawn_no_spin_landing(struct MarioState *m) {
    play_mario_landing_sound_once(m, SOUND_ACTION_TERRAIN_LANDING);
    set_mario_animation(m, MARIO_ANIM_GENERAL_LAND);
    stop_and_set_height_to_floor(m);
    if (is_anim_at_end(m)) {
...
Bool32 act_bbh_enter_spin(struct MarioState *m) {
    f32 floorDist;
    ...
    switch (m->actionState) {
        case ACT_BBH_ENTER_SPIN_STATE_SET_Y_VEL:
            floorDist      = (512.0f - (m->pos[1] - m->floorHeight));
            m->vel[1]      = ((floorDist > 0) ? (sqrtf(4.0f * floorDist + 1.0f) - 1.0f) : 2.0f);
            m->actionState = ACT_BBH_ENTER_SPIN_STATE_SPINNING;
            m->actionTimer = 100;
            // fall through
        case ACT_BBH_ENTER_SPIN_STATE_SPINNING:
            ...
            if (m->vel[1] <= 0) m->actionState = ACT_BBH_ENTER_SPIN_STATE_WAIT_FOR_ANIM;
            break;
        case ACT_BBH_ENTER_SPIN_STATE_WAIT_FOR_ANIM: // fall through
        case ACT_BBH_ENTER_SPIN_STATE_DIVE:
            m->faceAngle[1] = atan2s(cageDZ, cageDX);
            mario_set_forward_vel(m, forwardVel);
            m->flags &= ~MARIO_JUMPING;
            if (perform_air_step(m, 0) == AIR_STEP_LANDED) {
...
#endif
                m->actionState = ACT_BBH_ENTER_SPIN_STATE_END;
            }
            if (m->actionState == ACT_BBH_ENTER_SPIN_STATE_WAIT_FOR_ANIM) {
                if (m->marioObj->header.gfx.animInfo.animFrame == 0) m->actionState = ACT_BBH_ENTER_SPIN_STATE_DIVE;
            } else {
...
Bool32 act_bbh_enter_jump(struct MarioState *m) {
    ...
    perform_air_step(m, 0);
    if (m->vel[1] <= 0.0f) set_mario_action(m, ACT_BBH_ENTER_SPIN, 0);
    return FALSE;
}

Bool32 act_teleport_fade_out(struct MarioState *m) {
...
Bool32 act_teleport_fade_in(struct MarioState *m) {
    play_sound_if_no_flag(m, SOUND_ACTION_TELEPORT, MARIO_ACTION_SOUND_PLAYED);
    set_mario_animation(m, MARIO_ANIM_FIRST_PERSON);
...
Bool32 act_shocked(struct MarioState *m) {
...
Bool32 act_squished(struct MarioState *m) {
    f32 squishAmount;
    f32 spaceUnderCeil;
    Angle surfAngle;
    ...
    switch (m->actionState) {
        case ACT_SQUISHED_STATE_SCALING:
...
#endif
                m->actionState = ACT_SQUISHED_STATE_CHECK_HEIGHT;
            }
            break;
        case ACT_SQUISHED_STATE_CHECK_HEIGHT:
            if (spaceUnderCeil >= 30.0f) m->actionState = ACT_SQUISHED_STATE_UNSQUISH;
            break;
        case ACT_SQUISHED_STATE_UNSQUISH:
            m->actionTimer++;
            if (m->actionTimer >= 15) {
                // 1 unit of health
                if (m->health < 0x0100) {
...
Bool32 act_putting_on_cap(struct MarioState *m) {
...
static void intro_cutscene_land_outside_pipe(struct MarioState *m) {
    ...
    if (is_anim_at_end(m)) advance_cutscene_step(m);
    stop_and_set_height_to_floor(m);
}
...
static Bool32 act_intro_cutscene(struct MarioState *m) {
    switch (m->actionArg) {
...
static void jumbo_star_cutscene_falling(struct MarioState *m) {
    if (m->actionState == JUMBO_STAR_CUTSCENE_FALLING_STATE_FALL) {
        ...
        if (perform_air_step(m, 1) == AIR_STEP_LANDED) {
            ...
            play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
            m->actionState = JUMBO_STAR_CUTSCENE_FALLING_STATE_LAND;
        }
    } else {
        set_mario_animation(m, MARIO_ANIM_GENERAL_LAND);
        if (is_anim_at_end(m)) {
...
static void jumbo_star_cutscene_taking_off(struct MarioState *m) {
    struct Object *marioObj = m->marioObj;
    AnimFrame32 animFrame;
    if (m->actionState == JUMBO_STAR_CUTSCENE_TAKING_OFF_STATE_DANCE) {
        set_mario_animation(m, MARIO_ANIM_FINAL_BOWSER_RAISE_HAND_SPIN);
        marioObj->oMarioJumboStarCutscenePosZ = 0.0f;
        if (is_anim_past_end(m)) {
            play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
            m->actionState = JUMBO_STAR_CUTSCENE_TAKING_OFF_STATE_JUMPING;
        }
    } else {
...
static void jumbo_star_cutscene_flying(struct MarioState *m) {
    Vec3f targetPos;
    Angle targetPitch, targetYaw;
    switch (m->actionState) {
        case JUMBO_STAR_CUTSCENE_FLYING_STATE_INIT:
            set_mario_animation(m, MARIO_ANIM_WING_CAP_FLY);
            anim_spline_init(sJumboStarKeyframes);
            m->actionState = JUMBO_STAR_CUTSCENE_FLYING_STATE_ANGLES;
            // fall through
        case JUMBO_STAR_CUTSCENE_FLYING_STATE_ANGLES:
...
static Bool32 act_jumbo_star_cutscene(struct MarioState *m) {
    switch (m->actionArg) {
...
static void end_peach_cutscene_mario_landing(struct MarioState *m) {
    ...
    if (is_anim_at_end(m)) {
        ...
        m->capTimer = 60;
        sEndJumboStarObj = spawn_object_abs_with_rot(o, 0, MODEL_STAR, bhvStaticObject, 0, 2528, -1800, 0x0, 0x0, 0x0);
...
static void end_peach_cutscene_summon_jumbo_star(struct MarioState *m) {
    set_mario_animation(m, ((m->actionState == END_PEACH_CUTSCENE_SUMMON_JUMBO_STAR_STATE_RAISE_HAND) ? MARIO_ANIM_CREDITS_RAISE_HAND : MARIO_ANIM_CREDITS_LOWER_HAND));
    if ((m->actionState == END_PEACH_CUTSCENE_SUMMON_JUMBO_STAR_STATE_RAISE_HAND) && is_anim_past_end(m)) m->actionState = END_PEACH_CUTSCENE_SUMMON_JUMBO_STAR_STATE_LOWER_HAND;
    if (m->actionTimer ==  90) play_cutscene_music(SEQUENCE_ARGS(0, SEQ_EVENT_CUTSCENE_ENDING));
    if (m->actionTimer == 255) advance_cutscene_step(m);
...
static void end_peach_cutscene_spawn_peach(struct MarioState *m) {
    if (m->actionTimer ==  1) play_transition(WARP_TRANSITION_FADE_INTO_COLOR,  14, 255, 255, 255);
    if (m->actionTimer ==  2) play_sound(SOUND_MENU_STAR_SOUND, gGlobalSoundSource);
    if (m->actionTimer == 44) play_transition(WARP_TRANSITION_FADE_FROM_COLOR, 192, 255, 255, 255);
    if (m->actionTimer == 40) {
        obj_mark_for_deletion(sEndJumboStarObj);
        sEndPeachObj     = spawn_object_abs_with_rot(o, 0, MODEL_PEACH, bhvEndPeach,   0, 2428, -1300, 0x0, 0x0, 0x0);
        gCutsceneFocus   = sEndPeachObj;
        sEndRightToadObj = spawn_object_abs_with_rot(o, 0, MODEL_TOAD,  bhvEndToad,  200,  906, -1290, 0x0, 0x0, 0x0);
        sEndLeftToadObj  = spawn_object_abs_with_rot(o, 0, MODEL_TOAD,  bhvEndToad, -200,  906, -1290, 0x0, 0x0, 0x0);
...
static void end_peach_cutscene_descend_peach(struct MarioState *m) { // actionState is used as a timer
    ...
    if (sEndPeachObj->oPosY >= 1300.0f) {
        if (m->actionState <  60) m->actionState += 5;
    } else {
        if (m->actionState >= 27) m->actionState -= 2;
...
static void end_peach_cutscene_dialog_1(struct MarioState *m) {
    AnimFrame32 animFrame = set_mario_animation(m, ((m->actionState == END_PEACH_CUTSCENE_DIALOG_1_STATE_TAKE_OFF_CAP) ? MARIO_ANIM_CREDITS_TAKE_OFF_CAP : MARIO_ANIM_CREDITS_LOOK_UP));
    if (m->actionState == END_PEACH_CUTSCENE_DIALOG_1_STATE_TAKE_OFF_CAP) {
        if (animFrame == 8) cutscene_take_cap_off(m);
        if (is_anim_at_end(m)) m->actionState = END_PEACH_CUTSCENE_DIALOG_1_STATE_LOOK_UP;
    }
    switch (m->actionTimer) {
#ifdef VERSION_SH
        case 110: sEndPeachAnimation    = 6;                                                                                                                                                     break;
        case 111: sPeachManualBlinkTime = 3;                                                                                                                                                     break;
        case 175: sPeachManualBlinkTime = 2;                                                                                                                                                     break;
        case 258: sPeachManualBlinkTime = 1; sPeachIsBlinking =  TRUE;                                                                                                                           break;
        case 260: set_cutscene_message(160, 227, 0, 30); seq_player_lower_volume(SEQ_PLAYER_LEVEL, 60, 40); play_sound(SOUND_PEACH_MARIO,              sEndPeachObj->header.gfx.cameraToObject); break;
        case 305: sPeachManualBlinkTime = 0; sPeachIsBlinking = FALSE;                                                                                                                           break;
        case 320: set_cutscene_message(160, 227, 1, 60);                                                    play_sound(SOUND_PEACH_POWER_OF_THE_STARS, sEndPeachObj->header.gfx.cameraToObject); break;
        case 510: advance_cutscene_step(m);                                                                                                                                                      break;
#else
        case  80: sEndPeachAnimation    = 6;                           break;
        case  81: sPeachManualBlinkTime = 3;                           break;
        case 145: sPeachManualBlinkTime = 2;                           break;
        case 228: sPeachManualBlinkTime = 1; sPeachIsBlinking =  TRUE; break;
        case 230: set_cutscene_message(160, 227, 0, 30);
#ifndef VERSION_JP
            seq_player_lower_volume(SEQ_PLAYER_LEVEL, 60, 40);
            play_sound(SOUND_PEACH_MARIO, sEndPeachObj->header.gfx.cameraToObject);
#endif
            break;
        case 275: sPeachManualBlinkTime = 0; sPeachIsBlinking = FALSE; break;
        case 290: set_cutscene_message(160, 227, 1, 60);
#ifndef VERSION_JP
            play_sound(SOUND_PEACH_POWER_OF_THE_STARS, sEndPeachObj->header.gfx.cameraToObject);
#endif
            break;
        case 480: advance_cutscene_step(m);                            break;
#endif
    }
}
...
static void end_peach_cutscene_dialog_2(struct MarioState *m) {
    ...
    switch (m->actionTimer) {
        ...
        case 39:
            ...
            break;
        case 65: sPeachIsBlinking = TRUE; break;
        case 105:
#else
        case 29:
            set_cutscene_message(160, 227, 2, 30);
...
#endif
            break;
        case 45: sPeachIsBlinking = TRUE; break;
        case 75:
#endif
...
#endif
            break;
        case TIMER_PEACH_KISS: advance_cutscene_step(m); break;
    }
}
...
static void end_peach_cutscene_kiss_from_peach(struct MarioState *m) {
    ...
    if (m->actionTimer >= 90) m->marioBodyState->eyeState = ((m->actionTimer < 110) ? sMarioBlinkOverride[m->actionTimer - 90] : MARIO_EYES_HALF_CLOSED);
    switch (m->actionTimer) {
        case   8: sPeachIsBlinking      = FALSE;                        break;
        case  10: sPeachManualBlinkTime = 3;                            break;
        case  50: sPeachManualBlinkTime = 4;                            break;
        case  75: m->marioBodyState->eyeState = MARIO_EYES_HALF_CLOSED; break;
        case  76: m->marioBodyState->eyeState = MARIO_EYES_CLOSED;      break;
        case 100: sPeachManualBlinkTime = 3;                            break;
        case 136: sPeachManualBlinkTime = 0;                            break;
        case 140: advance_cutscene_step(m);                             break;
    }
}
...
static void end_peach_cutscene_star_dance(struct MarioState *m) {
    ...
    if (m->actionTimer <  52) m->marioBodyState->eyeState  = MARIO_EYES_HALF_CLOSED;
    switch (m->actionTimer) {
        ...
        case 140:
            ...
            break;
        case 142: advance_cutscene_step(m); break;
    }
}
...
#endif
            break;
        case  55: set_cutscene_message(160, 227, 6, 40); break;
#ifdef VERSION_JP
        case 130: set_cutscene_message(160, 227, 7, 50); break;
#else
        case 130: set_cutscene_message(160, 227, 7, 50); play_sound(SOUND_PEACH_FOR_MARIO, sEndPeachObj->header.gfx.cameraToObject); break;
#endif
    }
    if (m->actionTimer == 350) advance_cutscene_step(m);
}

// "Mario!"
static void end_peach_cutscene_run_to_castle(struct MarioState *m) {
    set_mario_animation(m, ((m->actionState == END_PEACH_CUTSCENE_RUN_TO_CASTLE_STATE_WALK_LOOK_UP) ? MARIO_ANIM_CREDITS_START_WALK_LOOK_UP : MARIO_ANIM_CREDITS_LOOK_BACK_THEN_RUN));
    m->marioObj->header.gfx.pos[1] = end_obj_set_visual_pos(m->marioObj);
    if ((m->actionState == END_PEACH_CUTSCENE_RUN_TO_CASTLE_STATE_WALK_LOOK_UP) && (is_anim_past_end(m))) m->actionState = END_PEACH_CUTSCENE_RUN_TO_CASTLE_STATE_LOOK_BACK_RUN;
    if (m->actionTimer == 95) {
...
static Bool32 act_end_peach_cutscene(struct MarioState *m) {
    switch (m->actionArg) {
...
static Bool32 act_credits_cutscene(struct MarioState *m) { // actionState is used as a timer
    ...
    if (m->pos[1] < m->waterLevel - 100) {
        ...
        set_mario_animation(m, MARIO_ANIM_FIRST_PERSON);
        if (m->actionTimer > 0) stop_and_set_height_to_floor(m);
    }
    if (m->actionTimer >= TIMER_CREDITS_SHOW) {
        ...
        sEndCutsceneVp.vp.vtrans[1] = (((gCurrCreditsEntry->posVpAndText & CREDITS_POS_THREE) ? height : -height) * 66 / 100) + (SCREEN_HEIGHT * 2);
        override_viewport_and_clip(&sEndCutsceneVp, 0, 0, 0, 0);
    }
    if (m->actionTimer   == TIMER_CREDITS_PROGRESS) reset_cutscene_msg_fade();
    if (m->actionTimer   >= TIMER_CREDITS_PROGRESS) sDispCreditsEntry = gCurrCreditsEntry;
    if (m->actionTimer++ == TIMER_CREDITS_WARP    ) level_trigger_warp(m, WARP_OP_CREDITS_NEXT);
    return FALSE;
}

static Bool32 act_end_waving_cutscene(struct MarioState *m) {
    if (m->actionState == ACT_END_WAVING_CUTSCENE_INIT) {
        m->statusForCamera->cameraEvent = CAM_EVENT_START_END_WAVING;
        sEndPeachObj                    = spawn_object_abs_with_rot(o, 0, MODEL_PEACH, bhvEndPeach,  60, 906, -1180, 0x0, 0x0, 0x0);
        sEndRightToadObj                = spawn_object_abs_with_rot(o, 0, MODEL_TOAD,  bhvEndToad,  180, 906, -1170, 0x0, 0x0, 0x0);
        sEndLeftToadObj                 = spawn_object_abs_with_rot(o, 0, MODEL_TOAD,  bhvEndToad, -180, 906, -1170, 0x0, 0x0, 0x0);
        ...
        sEndToadAnims[0]                =   6;
        sEndToadAnims[1]                =   7;
        m->actionState                  = ACT_END_WAVING_CUTSCENE_WAVING;
    }
...
Bool32 mario_execute_cutscene_action(struct MarioState *m) {
    ...
    switch (m->action) {
        ...
        case ACT_PULLING_DOOR:               // fall through
        ...
        default:                             cancel = act_exit_airborne(m);              break;
    }
    /* clang-format on */
