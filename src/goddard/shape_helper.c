...
void calc_face_normal(struct ObjFace *face) {
    ...
    if (face->vtxCount >= 3) {  // need at least three points to compute a normal
        struct ObjVertex *vtx1 = face->vertices[0];
        vec3_copy(p1, vtx1->pos);
        struct ObjVertex *vtx2 = face->vertices[1];
        vec3_copy(p2, vtx2->pos);
        struct ObjVertex *vtx3 = face->vertices[2];
...
void add_3_vtx_to_face(struct ObjFace *face, struct ObjVertex *vtx1, struct ObjVertex *vtx2, struct ObjVertex *vtx3) {
    vec3_set(face->vertices, vtx1, vtx2, vtx3);
    face->vtxCount    = 3;
...
s32 load_mario_head(void (*aniFn)(struct ObjAnimator *)) {
    ...
    d_use_int_names(TRUE);
    struct ObjAnimator *animator = (struct ObjAnimator *) d_makeobj(D_ANIMATOR, AsDynName(DYNOBJ_MARIO_MAIN_ANIMATOR));
    animator->controlFunc = aniFn;
    d_use_int_names(FALSE);
    // FIXME: make segment address work once seg4 is disassembled
    gMarioFaceGrp = (struct ObjGroup *) load_dynlist(dynlist_mario_master);
    // Make camera
    struct ObjCamera   *camera = (struct ObjCamera *) d_makeobj(D_CAMERA, NULL);
    Vec3f pos = { 0.0f, 200.0f, 2000.0f };
    d_set_rel_pos(  pos);
    d_set_world_pos(pos);
    d_set_flags(4);
    ...
    struct ObjParticle *particle = make_particle(GD_PARTICLE_FLAGS_NONE, COLOUR_WHITE, gVec3fZero);
    ...
    addto_group(gGdLightGroup, &particle->header);
    struct ObjGroup *mainShapesGrp  = (struct ObjGroup *) d_use_obj("N1000l"); // DYNOBJ_MARIO_MAIN_SHAPES_GROUP
    create_gddl_for_shapes(mainShapesGrp);
    struct GdObj     *objList       = gGdObjectList; // object list head before making a bunch of joints
    ...
    grabberJoint->rootAnimator      = animator;
    grabberJoint->header.drawFlags &= ~OBJ_IS_GRABBALE;
    struct ObjGroup *jointGrp       = make_group_of_type(OBJ_TYPE_JOINTS, objList);
    struct ObjNet *net              = make_net(jointGrp);
